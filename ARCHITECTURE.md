...
- Атрибуты сливаются только из ПРИНЯТОГО наблюдения. Среди них есть изменяемые (`is_cancel`, `supplier_status`, `wb_status`, `is_cancellable`), и безусловное слияние означало бы, что устаревшее сырьё, не сдвинув статус, всё равно их перепишет — заказ остался бы `CANCELLED` с `is_cancel = false`.
