...
- Порядок — отметка, потом объект. Обратный оставил бы при падении запись, утверждающую, что нагрузка на месте, тогда как её уже нет. При сбое между шагами остаётся осиротевший объект: он занимает место, но ничего не ломает, счётчик его показывает, путь уходит в лог ещё до коммита, а команда возвращает ненулевой код.
...
- Указатели на сырьё в `ingest_financial_transactions`, `ingest_order_status_events` и `ingest_orders.last_raw_record_id` после retention становятся неразрешимыми — это заявленное свойство, а не дефект: FK на сырьё нет, и история операций переживает своё доказательство.
