...
- Владение полями между потоками WB: `externalOrderId` принадлежит marketplace (номер заказа WB), `gNumber` из statistics — другая сущность и живёт только в атрибутах. Цену позиции задаёт marketplace: `finishedPrice` из statistics имеет другую семантику, и класть обе величины в одну колонку значило бы менять её смысл в зависимости от того, какой поток пришёл последним.
