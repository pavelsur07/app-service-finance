...
- У снимка и частичного наблюдения есть ВЛАДЕЛЕЦ (`snapshot_raw_record_id`, `partial_raw_record_id`, `Version20260902210000`). При равном времени повтор СВОЕГО сырья принимается, чужого — нет: у уже разобранного сырья отметка оси равна его собственному `fetchedAt`, и полный запрет отобрал бы у `forceReplay` его смысл — маппер починили, а цена и состав остались бы прежними. Обратное заполнение приближённое (`last_raw_record_id`) и ошибается в сторону отказа. Неизвестный владелец считается чужим.
