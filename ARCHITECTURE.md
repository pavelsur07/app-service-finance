...
- Номер маркетплейса WB обязан быть КАНОНИЧЕСКИМ (`^[1-9]\d{0,17}$`). Дальше строка становится ключом `int`, а поиск коллизий сравнивает строки: `5` и `05` дублями не считались, а ключ давали один — и один заказ молча затирал другой, оставаясь без наблюдения и без отметки попытки, то есть вечно первым в очереди.
