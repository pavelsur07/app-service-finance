...
- **Две фазы и две отметки**, потому что объектное хранилище не транзакционно. `payload_pruned_at` — РЕШЕНИЕ удалить, коммитится ДО обращения к хранилищу; `payload_deleted_at` — объект действительно удалён. Одной отметки не хватало: она ставится в памяти, а коммит происходит после удаления объекта, и падение между ними откатывало бы решение при уже уничтоженных данных. Незавершённое состояние видимо и повторяемо: следующий прогон находит помеченные, но не удалённые, и доводит дело до конца — без этой выборки объект остался бы навсегда, ведь кандидатов ищут среди непомеченных.
