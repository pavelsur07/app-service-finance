...
- Номер отправления Ozon проверяется ДО вызова клиента: пробельный идентификатор из базы считается `invalid`, получает отметку попытки и не прерывает прогон. Клиент на пустой номер бросает исключение, и без проверки один испорченный заказ обрывал всё: накопленные ответы не применялись, следующие подключения не обрабатывались, а сам заказ вечно стоял первым в очереди.
