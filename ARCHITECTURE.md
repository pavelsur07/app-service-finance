...
- Устаревшее наблюдение фиксируется как факт, но переходом не считается: у события `applied = false` и пустой `previousStatus`. Записанное иначе (`previousStatus = DELIVERED`, `status = SHIPPED`), оно утверждало бы движение заказа, которого не было.
