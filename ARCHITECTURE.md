...
- Идентичность позиции — `lineKey` (SKU или offerId плюс номер повторения), а не порядковый `lineNo`. Позиционный ключ ломается при перестановке `products` в ответе источника: строка сохраняла бы прежние опознавательные поля, но получала количество, цену и листинг соседнего товара. `lineNo` остаётся порядком отображения и обновляется вместе с содержимым.
...
- **Ограничение текущего охвата:** Ozon фильтрует отправления по времени СОЗДАНИЯ, поэтому почасовой обход видит заказ один раз и его последующие смены статуса не заметит. Отслеживание статусов обеспечивает отдельный цикл перепроса нетерминальных заказов (`app:ingestion:orders:refresh-statuses`), а не этот коннектор. До появления цикла история статусов заполнена лишь первым наблюдением.
