...
- Объект аудита, чью строку откатила транзакция записи наблюдений, УДАЛЯЕТСЯ компенсацией. Хранилище не транзакционно, а retention ищет кандидатов среди строк: объекта без строки он не найдёт никогда, и почасовой прогон превращал бы каждый повторяемый сбой в неограниченную утечку. Не удалось удалить — путь уходит в `error`: найти такой объект по базе невозможно.
