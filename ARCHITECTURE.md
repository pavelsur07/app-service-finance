...
- История событий при РАВНОМ `observedAt` идёт в порядке ПРИМЕНЕНИЯ: тай-брейк — `id` события (UUID v7, создаётся под блокировкой заказа в момент применения), а не `rawRecordId`. Сырьё получает идентификатор при загрузке, задолго до разбора и другим процессом, и его порядок с порядком применения не связан — цепочка `previousStatus` не сходилась.
