...
- **Retention — год горячего хранения, удаляет приложение** (`app:ingestion:raw:prune`, cron `50 5 * * *`). Lifecycle-правило провайдера не годится: оно видит только объекты и снесло бы их, не оставив признака, что нагрузка удалена НАМЕРЕННО, — `ReadRawRecordAction` падал бы невнятной ошибкой хранилища.
- **Удаляется НАГРУЗКА, строка остаётся** (`payload_pruned_at`, `Version20260902180000`). Первая версия удаляла пару «объект + строка», и это порождало класс гонок, каждая из которых закрывалась правкой очередной подсистемы: запись исчезала между проверкой и созданием `NormalizationIssue`, а дедуп при часовом опросе обновлял `lastSeenAt` у уже удалённой строки — свежая выгрузка терялась молча, потому что UPDATE задевал ноль строк. Дорого стоит объект, а не сотня байт метаданных. Со строкой на месте: указатели из `ingest_financial_transactions`, `ingest_order_status_events` и `ingest_orders.last_raw_record_id` разрешаются; чтение отвечает `RawPayloadPrunedException`, а не сбоем хранилища; дедупу есть что обновлять; а `StoreRawBatchAction::restorePayload()` возвращает нагрузку и снимает отметки, если та же выгрузка приедет снова — под блокировкой той же строки и строго в порядке «сначала объект, потом снятие отметки».
- Возраст считается по `lastSeenAt`, а не по `fetchedAt`. Дедуп при часовом опросе не создаёт новую запись, а обновляет отметку «видели»: удалив запись, скачанную год назад, но подтверждаемую каждый час, мы получили бы её обратно следующим же прогоном под новым идентификатором. Свежесть — это когда последний раз видели.
- **Три отметки и две фазы**, потому что объектное хранилище не транзакционно. `payload_pruned_at` — РЕШЕНИЕ удалить, коммитится ДО обращения к хранилищу; `payload_deleted_at` — объект действительно удалён. Одной отметки не хватало: она ставится в памяти, а коммит происходит после удаления объекта, и падение между ними откатывало бы решение при уже уничтоженных данных. Третья, `payload_deletion_attempted_at`, спасает очередь от голодания: она сортируется по времени попытки, иначе неустранимый объект вечно занимал бы её начало. Незавершённое состояние видимо и повторяемо: следующий прогон находит помеченные, но не удалённые, и доводит дело до конца — без этой выборки объект остался бы навсегда, ведь кандидатов ищут среди непомеченных. Backlog обслуживается ПЕРВЫМ и из общего бюджета прогона, иначе новые решения вытесняли бы его всё дальше.
- Отменить решение можно только при ЖИВОМ объекте. Состояние «решение принято, объект уже удалён, коммит не прошёл» достижимо, и слепая отмена вернула бы запись к виду «нагрузка есть» при отсутствующем объекте: чтение падало бы ошибкой хранилища, а проблема всё равно осталась бы без доказательства — только теперь молча. Если объекта нет, запись закрывается как очищенная, а `error` сообщает, что проблему придётся разбирать без доказательства.
- `bytesFreed` считает лишь объекты, которые к моменту удаления ещё были на месте: повтор после сбоя коммита подтверждает отсутствие, а не освобождает что-то ещё.
- Вторая фаза тоже идёт под блокировкой строки: повторная выгрузка могла вернуть нагрузку и снять отметки, пока прогон шёл к хранилищу. Чанк в ней меньше первой — внутри транзакции сетевые вызовы. Если удаление не удалось, отметка решения остаётся, а отметка удаления нет: следующий прогон повторит попытку, объект пока занимает место, его размер в освобождённые не идёт, путь уходит в лог, команда возвращает ненулевой код.
- Индексы retention строятся `CREATE INDEX CONCURRENTLY` отдельной нетранзакционной миграцией: `ingest_raw_records` горячая, и обычный `CREATE INDEX` остановил бы ingestion на время построения.
- Сырьё, на которое ссылается НЕРАЗОБРАННАЯ `NormalizationIssue`, не очищается: оно и есть то, с чего начинает разбирающий. Удержание проверяется ТРИЖДЫ и в разных местах, потому что «проблемы нет» — это отсутствие строки, а его блокировка сырья сама по себе не запрещает: при отборе кандидатов, под блокировкой в фазе решения и снова под блокировкой в фазе удаления — там появившаяся проблема ОТМЕНЯЕТ решение целиком. Замыкает протокол `RecordNormalizationIssueAction`: он блокирует ту же строку сырья перед созданием проблемы, поэтому две операции становятся последовательными. Удержание перепроверяется УЖЕ ПОД блокировкой строки сырья и отдельным запросом: `FOR UPDATE` защищает строку, но не отсутствие строки в таблице проблем, и условие внутри блокирующей выборки осталось бы снимком её собственного момента. **Гарантия ровно такая:** проблема, закоммиченная до перепроверки, всегда сохраняет свою нагрузку. Проблема, закоммиченная между перепроверкой и коммитом, нагрузки лишится — но не указателя: запись на месте и сообщает, когда нагрузка удалена. Это видимая деградация, а не тихая потеря, и именно ради неё строка переживает свою нагрузку. Состояние не вечное — закрытая проблема отпускает запись следующим прогоном, — но удержание считается отдельным счётчиком и попадает в `warning`: иначе «нечего удалять» и «удалять нельзя» выглядели бы одинаково.
- Указатели на сырьё в `ingest_financial_transactions`, `ingest_order_status_events` и `ingest_orders.last_raw_record_id` после retention ОСТАЮТСЯ разрешимыми: строка на месте. Недоступна только полезная нагрузка, и запись сама сообщает, когда её удалили.
- Удаление необратимо, поэтому умолчания у команды нет: `--dry-run` или `--execute`, ровно одно. `--limit` ограничивает длительность прогона; накопленное разбирается за несколько суток.
- Удержание считается ПО ПЛАНУ, до всякого исполнения, и включает удержания среди уже помеченного backlog: запрос удержаний смотрит только на непомеченные записи, и backlog выпадал из прогноза целиком. `pendingBytes` считает только то, что действительно освободится. Удержания, появившиеся ПОСЛЕ построения плана, идут отдельным числом (`heldAfterPlanning`) — сложить их вместе значило бы посчитать одну запись дважды на execute и не посчитать вовсе на dry-run.
- Восстановление после гонки уникального индекса (`recoverConcurrentDuplicate`) идёт ПОД ТОЙ ЖЕ блокировкой, что и обычная повторная встреча. Прежнее рассуждение «запись только что создана конкурентом, значит свежая» кодом не обеспечено: ветка не смотрит на `lastSeenAt`, а историческая выгрузка попадает под retention-предикат сразу.
- Повторная встреча сырья не восстанавливает нагрузку, если строки под блокировкой не оказалось: прежний откат на прочитанную ранее сущность писал объект без блокировки, а UPDATE задевал ноль строк — вызывающий получал «сохранено» при записи, которой нет, и в хранилище оставалась сирота.
- Очистка ОТКАЗЫВАЕТСЯ работать внутри уже открытой транзакции. Обе фазы опираются на то, что решение коммитится до обращения к хранилищу, а внутри чужой транзакции `wrapInTransaction()` даёт лишь вложенный уровень: «коммит» первой фазы ничего не фиксирует, и откат внешней транзакции после успешного удаления вернул бы строке вид записи с нагрузкой, которой уже нет. По этой же причине её тесты идут без DAMA-отката (`PostgresResetTestCase`): под ним они проверяли бы не ту семантику, которую защищают.
- Прогон retention и откат retention-миграций держат ОБЩУЮ advisory-блокировку (`pg_advisory_lock(6902180000)`). `ACCESS EXCLUSIVE` внутри транзакции защищает только собственный шаг, а между `down()` двух миграций блокировок нет вовсе — и прогон успевал пометить и удалить нагрузку ровно там: индексы уже сняты, а следующая миграция прерывается на своей проверке. Прогон берёт замок через `pg_try_advisory_lock` и, не получив, пропускает ночь с `warning`: откат — операция человека на минуты, а retention идёт каждую ночь.
- Откат миграции индексов идёт СВОЕЙ транзакцией под `ACCESS EXCLUSIVE` и обычным `DROP INDEX`, без `CONCURRENTLY`. Асимметрия с `up()` намеренная: долгая операция — построение индекса, ради неё `CONCURRENTLY` и нужен; удаление — операция над метаданными. А проверка и удаление обязаны быть одним решением: миграция нетранзакционная, и между `COUNT(*)` и `DROP` конкурентный `--execute` успевал закоммитить отметку.
- Откат обеих retention-миграций отказывается работать, пока есть хоть одна помеченная запись, и `Version20260902180000::down()` берёт `ACCESS EXCLUSIVE` ДО подсчёта: проверка и DDL иначе разнесены во времени, и прогон успевал закоммитить отметку между ними. Проверка стоит и в `Version20260902190000::down()`, потому что откат идёт от новых миграций к старым и снял бы индексы горячей таблицы раньше, чем следующая миграция прервалась бы.
- Сбой хранилища ловится как `\Exception`, а не `\Throwable`. `TypeError`, `AssertionError` и прочие `\Error` — дефекты программы: следующий прогон их не исправит, и превращать их в ожидаемый retry значило бы спрятать поломку за `warning` и вечным pending.
- Индекс backlog содержит ВЫРАЖЕНИЕ `CASE` из `ORDER BY`, а не только колонку времени попытки: планировщик не знает, что `CASE WHEN attempted IS NULL THEN 0 ELSE 1 END, attempted` — это `attempted NULLS FIRST`. Проверено на 200 000 помеченных записей запросом той формы, которую строит Doctrine: без выражения индекс не использовался вовсе (Parallel Seq Scan + top-N, 5899 буферов, 40 мс), с ним — Index Scan, 28 буферов, 0,2 мс. Именно Index Scan, а не Index Only: ORM гидратирует сущность целиком, и индекс даёт порядок с лимитом, а не покрытие.
- **План строится одинаково в обоих режимах.** Незавершённое прошлых прогонов идёт первым и из того же лимита, поэтому новые кандидаты отбираются по ОСТАТКУ бюджета, а `pendingRetries`/`pendingBytes` показываются отдельно от `candidates`/`candidateBytes`. Иначе dry-run описывал бы не тот прогон, который потом выполнится: он молчал бы о работе, которую execute сделает раньше всего, и обещал бы удалить кандидатов, до которых бюджет не дойдёт.
- Проблема на несуществующую в этой компании строку сырья НЕ заводится (`error`). Предикат удержания сопоставляет компанию, потому что блокируется пара `(компания, сырьё)`: проблема, не взявшая эту блокировку, протоколом не сериализована, и считать её удержанием значило бы позволить одному арендатору тормозить очистку другого.
- **В кроне стоит `--dry-run`.** Ночной прогон печатает, сколько записей и байт (`candidateBytes`) ушло бы, и ничего не удаляет. Переключение на `--execute` — решение Владельца на Production Gate: удаление необратимо и затрагивает production-аудит, а такое не должно включаться побочным эффектом выкатки стадии. Сначала настоящие цифры, потом решение.
...
- Ozon accrual taxonomy replaces the generic `OTHER` fallback for known `NON_ITEM` fees. Every Ozon accrual normalization, forced or not, voids obsolete same-raw `(externalId, type)` components before control-sum verification, so changing `OTHER` to the semantic type cannot leave two active amounts; if the taxonomy later reverts, a same-raw replay may reactivate the previously voided natural key at the same source version. `app:ingestion:ozon-accrual:normalize-stored --include-done` is allowed only with `--execute-inline`; its queued mode is rejected because the replay flag would be lost. Maintenance entry points `app:ingestion:ozon-accrual:refresh-financial-verification` and `app:ingestion:ozon-accrual:reconcile-financial-projection` may reset selected records and normalize them without the force flag; the reconcile command also has a daily cron path with queued normalization. The unconditional same-raw Ozon voiding keeps all of these paths convergent. Ozon and WB inline force replay do not demote an already valid record to `PENDING` before the normalization transaction: interruption or mapper failure preserves its previous `DONE` projection, records the issue, and still returns a failed command outcome.
- Deployment does not rewrite rows directly, but the daily Ozon financial-projection reconcile job may automatically reprocess records selected by its health query after deployment. A complete historical repair still requires an explicitly scoped inline replay (`--company-id`, `--from`, `--to`, `--include-done`, `--execute-inline`) for the corresponding Ozon or WB command; running it outside an isolated local/test environment is a separate Production Gate.
- WB finance deductions preserve the provider reason in `sourceData`. `deduction > 0` maps to `ADJUSTMENT/OUT`, `deduction < 0` to `ADJUSTMENT/IN`; known voluntary goods compensation and warehouse disposal reasons also receive stable category code, label, and group metadata.
...
- События журнала различаются ПОРЯДКОВЫМ НОМЕРОМ внутри пары (сырьё, заказ), а не содержанием. Содержательного ключа, свободного от дефекта, не существует: одно сырьё может нести A → B → A → B, и любой набор колонок статуса подавит одно из повторяющихся наблюдений — заказ переход применит, а журнал его потеряет. От повторной записи защищает не индекс, а транзакция разбора и отдельный путь повтора (`reapply()`).
- Между подключениями карта идентичности очищается: иначе память растёт как ОБЩЕЕ число обработанных заказов, а не как лимит на подключение, и после OOM следующий прогон снова начинается с первых подключений — последние голодали бы постоянно.
- Неожиданный HTTP-код останавливает подключение, а нарушение формы успешного ответа — только одно отправление. Смешивать нельзя в обе стороны: сломанный эндпоинт дал бы сотни одинаковых запросов и успешный прогон, а одно кривое отправление каждый час останавливало бы обработку всех следующих.
...
- Очередь перепроса опирается на частичный индекс `idx_ingest_order_refresh_queue` (`Version20260902170000`): `LIMIT` ограничивает результат, но не число строк, которые PostgreSQL обязан отфильтровать и отсортировать на каждое подключение. Индекс включает выражение `CASE` из `ORDER BY` и исключает терминальные статусы — заказ, созданный сразу доставленным, имеет пустую отметку попытки и занимал бы начало индекса. Проверено `EXPLAIN (ANALYZE)` на 200 000 заказов (20 компаний, 40 подключений): индекс используется, при упорядоченном сканировании сортировки нет вовсе — 205 буферов и 1,1 мс против 4039 буферов и 10,7 мс у bitmap-плана с top-N сортировкой.
...
- Порядок событий журнала при равном `observedAt` задаёт `recorded_seq` — номер, выданный счётчиком САМОГО ЗАКАЗА (`ingest_orders.status_event_seq`) под его блокировкой (`Version20260902200000`). Ни `raw_record_id`, ни `id` события для этого не годятся: сырьё получает идентификатор при загрузке, задолго до разбора и другим процессом, а UUID v7 упорядочен лишь по миллисекунде — два процесса, взявшие блокировку внутри одной миллисекунды, могли дать порядок, обратный порядку применения. Счётчик у заказа, а не глобальная последовательность: монотонность нужна в пределах одного заказа, а общая последовательность добавила бы точку соперничества между всеми компаниями.
- Неустранимый сбой подключения (`brokenConnections`) отделён от retryable: ответ, нарушающий контракт эндпоинта, через час будет ровно таким же. Он даёт `error` и НЕНУЛЕВОЙ код возврата команды — иначе постоянная несовместимость API под `--quiet` в кроне выглядела бы как успешный прогон при подключении, которое не обновляется вовсе. 429 и таймаут остаются `warning` и нулевым кодом: они лечатся следующим прогоном.
- `updatedAt` заказа НИКОГДА не едет назад (`max(current, writeTime)`): момент операции снимается до ожидания блокировки, а под ней перечитывается состояние, которое конкурент мог записать позже. Своё время остаётся в собственных колонках, а выборка «что изменилось с такого-то момента» не теряет чужую свежую запись. Время остановки зависших снимается ПОСЛЕ получения блокировок — ожидание может занять минуты.
- У снимка и частичного наблюдения есть ВЛАДЕЛЕЦ (`snapshot_raw_record_id`, `partial_raw_record_id`, `Version20260902210000`). При равном времени повтор СВОЕГО сырья принимается, чужого — нет: у уже разобранного сырья отметка оси равна его собственному `fetchedAt`, и полный запрет отобрал бы у `forceReplay` его смысл — маппер починили, а цена и состав остались бы прежними. Исторические заказы владельца НЕ получают: обратное заполнение из `last_raw_record_id` выглядело безопасным и таким не является — этот указатель двигается только принятым статусом, а снимок применяется и без статуса, поэтому заполнение назвало бы владельцем чужое сырьё и разрешило бы ему откатить чужие данные. Неизвестный владелец считается чужим, то есть исторические заказы сохраняют прежнее осторожное поведение.
- **Выкатка этих миграций требует ОСТАНОВКИ ingestion-воркеров, а не rolling deployment.** Уникальный индекс журнала переезжает с `(company, raw, order, raw_status)` на `(company, raw, order, occurrence)`, а воркер прежней версии `occurrence` не пишет вовсе: законная пара A → B из одного сырья получит у него два нуля и нарушит новый индекс. Разбор такого сырья откатится и уйдёт в повтор — данные не теряются, но пока старые воркеры живы, часть сообщений будет падать. Тот же старый воркер не берёт блокировок нового протокола и способен перезаписать состояние, только что записанное перепросом. Порядок: остановить (или дренировать) ingestion-воркеры → применить миграции → выкатить код → включить почасовой крон перепроса.
- Значение, записанное `datetime_immutable_us`, ЧИТАЕТСЯ прежним стандартным типом: `createFromFormat('Y-m-d H:i:s', …)` разбирает дробную часть как хвост и возвращает объект. Это про выкатку — rolling deployment не роняет normalize-worker'ы прежней версии на гидратации общей таблицы сырья. Проверено тестом, а не рассуждением.
- ПОВТОР уже разобранного сырья не принимает равное время НИ ПО ОДНОЙ оси — ни статусной, ни снимочной, ни частичной. Для наблюдения «последний победил» осознанное правило, для повтора — нет: он ничего нового не сообщает, а применённый заново откатывал бы цену, валюту, атрибуты и состав, записанные другим сырьём того же мгновения. Статусная ось была закрыта отдельно, и в этом была ловушка: заказ выглядел согласованным по статусу, а порча остальных данных ничем себя не выдавала.
- Объект, записанный до провалившегося `flush()` или коммита, НЕ удаляется — его путь уходит в `error`. Соблазн удалить синхронно был и оказался опасен: исход коммита бывает неопределённым — PostgreSQL мог зафиксировать строку, а клиент потерять подтверждение, — и удаление оставило бы живую запись без нагрузки и без отметки `payload_pruned_at`, то есть необратимую потерю вместо занятого места. Правило одно для `StoreRawBatchAction` и для аудита перепроса. Сирота при этом видима: retention ищет кандидатов среди строк и объекта без строки не найдёт никогда, поэтому убрать его может только человек по пути из лога, убедившись, что строки на него нет. Автоматический reconciler с доказательством отсутствия владельца без TOCTOU — FOLLOW-UP.
- Проблемы разбора записываются ПАЧКОЙ (`recordMany`) за ДВА запроса, а не за два на проблему: поштучный вызов брал бы блокировку сырья и читал отметки на каждую, а прогон заводит их до тысячи за раз и держит при этом блокировки всей пачки. Первый запрос отвечает, КОМУ принадлежит каждая строка, и делает это без блокировки; второй блокирует и читает отметки — в глобальном порядке по идентификатору. Авторизуется ПАРА «компания + строка», а не идентификатор: список идентификаторов позволял бы честной команде своей компании открыть дорогу поддельной команде того же идентификатора под чужой компанией. Порядок глобальный потому, что иначе пачка и прогон retention берут строки в разной последовательности и складываются в цикл; а фильтр по компании внутри блокирующего запроса этот порядок разрушил бы. Чужую строку нельзя блокировать даже на мгновение — это межтенантный побочный эффект, задерживающий чужой ingestion, и компания у строки неизменяема, поэтому предварительная проверка владения не устаревает. Размер пачки ограничен явно (1000, наибольший `--limit` прогона): блокировки живут до конца транзакции вызывающего, и неограниченная пачка остановила бы ingestion надолго.
- Токен статуса с NUL (`"deliver\u0000ed"` — валидный JSON) бракуется в обоих валидаторах, Ozon и WB, причём проверка стоит ДО `trim()`: стандартный `trim()` срезает `\0` по краям, и `\0delivered` превращался бы в допустимый `delivered` — повреждённый ответ принимался бы как настоящий терминальный статус. PostgreSQL не принимает `0x00` в text: такое значение проходило по типу, `trim()` и длине, доезжало до финального flush и откатывало общую транзакцию подключения вместе с уже собранными наблюдениями соседей. Сырьё сохраняется до разбора, доказательство остаётся.
- Ответ WB применяется только к заказу с ТЕМ ЖЕ номером маркетплейса, что спрашивали: номер фиксируется ДО сети для КАЖДОГО спрашиваемого заказа (отметка попытки ставится и при промахе, и при браке, и при сбое — все они о номере, который спрашивали) и сверяется под блокировкой с перечитанным заказом. Для Ozon так же фиксируется СХЕМА: по ней выбирали эндпоинт, и ответ старого пути не приписывается переклассифицированному заказу. Между запросом и записью проходят минуты, и параллельная нормализация может сменить `externalOrderId`; без сверки ответ для номера 5 ложился бы на заказ, который теперь связан с номером 7, в журнал попадал бы чужой переход, а терминальный результат навсегда закрывал бы перепрос. Несовпадение откладывает заказ целиком — без отметки попытки и без наблюдения: попытка была не о нём.
- Присвоение и применение номера маркетплейса СЕРИАЛИЗОВАНЫ транзакционным advisory-замком на подключение (`IngestOrderRepository::lockConnectionScope`, `pg_advisory_xact_lock(hashtext(company:source:connection))`). Нормализация берёт его первым — до сырья и заказов, перепрос — до блокировки заказов; порядок «advisory → сырьё → заказы» один для всех. Под ним перепрос ПЕРЕПРОВЕРЯЕТ коллизии непосредственно перед записью: проверка до сети устаревала за минуты запроса, и ответ на номер, присвоенный тем временем ещё одному заказу, ложился на первого носителя — вплоть до терминального статуса. Уникального ограничения на `external_order_id` нет намеренно: коллизии есть в данных, их видят и откладывают.
- Пакетная запись проблем логирует «подготовлено», а не «записано»: `wrapInTransaction()` изнутри чужой транзакции — лишь вложенный уровень, и утверждать «записано» вправе только тот, кто коммитит.
- Номер отправления Ozon проверяется ДО вызова клиента и обязан быть КАНОНИЧЕСКИМ — непустым и без пробелов по краям; иначе заказ считается `invalid`, получает отметку попытки и не прерывает прогон. Клиент номер НЕ нормализует: срезав пробелы, он спросил бы «P-1» вместо « P-1 » и сверил бы номер ответа уже с обрезанным — статус чужого отправления вернулся бы как ответ на наш заказ. Неканонический номер для клиента — ошибка вызывающего. Клиент на пустой номер бросает исключение, и без проверки один испорченный заказ обрывал всё: накопленные ответы не применялись, следующие подключения не обрабатывались, а сам заказ вечно стоял первым в очереди.
- Остановка зависших блокирует только СВОЁ сырьё: владелец выясняется до блокировки (`ownersOf`), а заказ, чей указатель ведёт на сырьё другой компании, останавливается как заказ без доказательства. Указатель — данные, они могут быть повреждены, и `PESSIMISTIC_WRITE` на чужую строку задерживал бы чужой ingestion.
- `substatus` Ozon при перепросе — УТОЧНЕНИЕ и в нормализацию не попадает; непригодное уточнение отбрасывается с `warning`, а статус применяется. Пока оно отклоняло весь ответ, валидный переход — в том числе терминальный — не попадал ни в заказ, ни в журнал, а отметка попытки сдвигалась: заказ доезжал до STUCK_ORDER из-за поля, которое на статус не влияет. Принятое наблюдение при этом ЗАМЕНЯЕТ уточнение, а не оставляет прежнее при пустом: `delivered` рядом с `posting_on_way_to_city` — противоречие, которого маркетплейс не присылал.
- Нормализация тоже пишет проблемы пачкой (`recordMany`, чанками по 1000) перед финальным flush, а не по одной внутри цикла: партия до 20 000 заказов давала бы десятки тысяч вложенных транзакций и запросов под блокировками сырья и всех заказов партии — пока часовой перепрос ждёт те же строки.
- Незнакомый токен статуса попадает в видимую очередь, когда наблюдение ЗАПИСАНО в журнал, а не когда оно сдвинуло состояние. Наблюдение, проигравшее более свежему, состояние не двигает, но событие пишет — и токен в нём такой же настоящий; пока условием было «изменилось», сломанный контракт API оставался только в журнале, а если победившее наблюдение сделало заказ терминальным, второй попытки не было бы никогда.
- Тело WB, которое не является JSON или объектом, — свойство ЭНДПОИНТА (`endpointWide`, ненулевой код возврата), а разобранный скаляр идёт в сырьё через конверт. Прежде полностью несовместимый ответ выглядел как успешный часовой прогон.
- Строка статуса со строковым номером (`"id": "5"`) ОПОЗНАЁТСЯ, хотя и бракуется: заказ она называет однозначно. Пока опознание требовало `int`, такой заказ считался и `invalid`, и `missing`, а в аудит уходило ложное «маркетплейс его не вернул».
- Зависший заказ, чей указатель на сырьё цел, а строки сырья нет, останавливается — не откладывается. Откладывание было вечным: строка не появится, а заказ старейший и при малом лимите занимал бы начало очереди зависших всегда. Отличается это от конкурентной смены указателя картой «заказ → сырьё на момент отбора».
- Ответ на запрос статусов WB разбирается СТРОГО к контейнеру и терпимо к строкам: `orders`, не являющийся списком, — свойство эндпоинта (`endpointWide`, ненулевой код возврата), а не-объектная строка внутри корректного списка отбраковывается поштучно. Прежде одна такая строка уносила все корректные наблюдения соседей, а поломка контейнера считалась дефектом пачки и заканчивалась успешным прогоном.
- Две строки одного номера бракуют ОБЕ: счётчик отбракованных строк учитывает и ту, что была принята до повтора.
- Верхняя граница номера маркетплейса — `PHP_INT_MAX`, а не число цифр: проверяется обратным преобразованием `(string) (int) $id === $id`. Предел в 18 цифр отвергал бы настоящий девятнадцатизначный номер навсегда, доводя заказ до STUCK_ORDER, а переполнение молча даёт `PHP_INT_MAX`.
- Доказательством считается любой РАЗОБРАННЫЙ ответ, а не только JSON-объект. `[]`, `"broken"` и `0` нарушают контракт ровно так же; не-объект заворачивается в конверт `_malformed_response`, потому что строка сырья — объект на запись. Пустое доказательство означало бы, что сырья не появится вовсе и разбирать дефект интеграции будет не по чему. `null` остаётся `null`: разбирать было нечего.
- Номер маркетплейса WB обязан быть КАНОНИЧЕСКИМ положительным десятичным числом, помещающимся в `PHP_INT_MAX`: `^[1-9]\d{0,18}$` И обязательная проверка обратным преобразованием `(string) (int) $id === $id`. Дальше строка становится ключом `int`, а поиск коллизий сравнивает строки: `5` и `05` дублями не считались, а ключ давали один — и один заказ молча затирал другой, оставаясь без наблюдения и без отметки попытки, то есть вечно первым в очереди. Граница именно `PHP_INT_MAX`, а не число цифр: предел в 18 цифр отвергал бы настоящий девятнадцатизначный номер навсегда, а переполнение молча даёт `PHP_INT_MAX` — обратное преобразование единственная точная проверка.
- Коллизия номеров ищется среди ВСЕХ заказов подключения, включая терминальные и остановленные. Один номер маркетплейса — один заказ маркетплейса; неоднозначность создаёт сам факт двух носителей, а не то, опрашиваем ли мы второго. Пока область совпадала с очередью, пара «нетерминальный + терминальный» давала `COUNT = 1`, и ответ единственного заказа маркетплейса приписывался, возможно, не тому.
- Две строки ОДНОГО заказа в ответе WB бракуют весь номер, а не лишнюю строку. Оставить первую значило бы выбрать статус по порядку в ответе: строки противоречат друг другу, и если произвольно выбранная окажется терминальной, заказ навсегда выпадет из перепроса.
- В сырьё уходят строки ОТВЕТА (`WbOrderStatusPage::$auditRows`), а не наш разбор. В `statuses` оси нормализованы, и запись, объявленная сырым ответом маркетплейса, содержала бы наши значения вместо чужих — отклонение вроде `" complete "` после такой записи уже не восстановить, а разбирают потом именно его.
- Блокировки берутся в одном порядке всюду: СНАЧАЛА сырьё, ПОТОМ заказы. Порядок задаёт нормализация — она иначе не может, разбор идёт от сырья. Часовая остановка зависших брала их наоборот, и две транзакции складывались в цикл: крон держал заказ и ждал сырьё, нормализатор держал сырьё и ждал заказ. PostgreSQL разрывает цикл, убивая одну из транзакций: пачка остановки откатывалась целиком, а часовой прогон падал. Заказ, у которого сырьё сменилось между выборкой и блокировкой, откладывается до следующего прогона.
- Зависший заказ БЕЗ сырья тоже останавливается — просто без записи в очереди на разбор, и об этом кричит `error`. Пока его пропускали, получалось обратное задуманному: окно опроса его уже не захватывало, а выборка зависших отсеивала, — заказ не обновлялся, не останавливался и никуда не попадал, а лимит остановки тратился на него впустую каждый прогон.
...
- **Тип `datetime_immutable_us` отвечает и за ЗОНУ.** Колонка без зоны хранит голые «стенные часы», поэтому момент приводится к зоне приложения на записи и разбирается в ней же на чтении. Раньше значение писалось в зоне ОБЪЕКТА: отметка из `ClockInterface` (UTC) ложилась в базу как UTC и читалась как местная — каждый round-trip терял смещение зоны. На заказах это делало наблюдение «старше самого себя», а в UI верификации показывало время скачивания сырья на три часа раньше, чем оно было.
...
- Коды затрат WB частично динамические: `WbDeductionCalculator` слугифицирует неизвестные названия удержаний и режет их до 50 символов. Известные причины получают стабильные коды: добровольная выплата за товары — `wb_dobrovolnaya_vyplata_za_tovary`, складская утилизация — `wb_warehouse_disposal`. `external_id` сохраняет прежнюю reason-based идентичность, чтобы replay старого отчёта не создал дубль с новым кодом категории. Старые помесячные коды утилизации остаются в конфиге как исторические aliases до ремонта данных.
- Рекламные удержания WB с постоянным текстом («WB Медиа», подписка «Джем», аванс «Баллы за отзывы») внесены в `WbCostCategory` под теми же кодами, что даёт слугификация: `forDeductionName()` для них не нужен, группа аналитики — «Продвижение и реклама». «Добровольная выплата за товары» намеренно без правила ОПиУ (решение Владельца 26.09.2026).
- Услуги Ozon by-day разбираются по `OzonCostCategory::$accrualTypeNames`. Каждая услуга снимка справочника `tests/Fixtures/Marketplace/Ozon/accrual_types.json` либо размечена, либо стоит в `OzonCostCategoryTest::AWAITING_CLASSIFICATION` — тест падает на услуге без решения. Размечать только при однозначной категории, которая уже получала деньги; сомнительная услуга остаётся `ozon_unknown_<id>`: видимая очередь безопаснее тихой ошибки в строке ОПиУ.
...
- **Ни одна ветка не завершается молча.** Дубль по `external_id` и неполная идентичность
  сообщения тоже отвечают пользователю в чат: 200 без ответа неотличим снаружи от
  мёртвого бота, и именно так выглядел инцидент 13.09.2026.
- Логи вебхука идут в **отдельный канал Monolog `telegram`** (не в общий `app`). На проде
  у канала свой stream-handler уровня `info`, потому что `main` — `fingers_crossed` с
  `action_level: error`, и INFO из него до stderr не доходит. `ERROR` канала по-прежнему
  попадает в Sentry/GlitchTip: sentry-хендлер не ограничен списком каналов
  (инвариант `SentryChannelCoverageTest`).
- Уровни сбоев Bot API: `403`/`429` — `WARNING` (проходят сами: бот заблокирован, flood
  control); всё остальное, включая `400`, `401`, `5xx`, ответ без `ok:true` и ответ не от
  Telegram (шлюз `tg-gateway`, прокси) — `ERROR`, потому что ответа не получит уже никто.
- В логи канала не попадают: текст сообщения пользователя (суммы, контрагенты, назначения
  платежей), токен привязки из `/start` и токен бота — он лежит в URL Bot API и вырезается
  из текста исключений перед логированием.
...
Готовые блоки конфигурации нового модуля — docs/workflow/new-module.md
...
запросах. Составные FK `(company_id, connection_id)` от всех трёх таблиц
запрещают смешивать компании и удалять подключение с данными.
...
Следующий входящий поток хранит `MoySkladProduct` и `MoySkladVariant` только
в модуле MoySklad. У обоих отдельная уникальность `(connection_id, external_id)`;
вариант имеет FK `(company_id, connection_id, product_external_id)` на товар
того же подключения. Сохраняются идентичность, имя, коды, архив, время источника
и загрузки; у товара `variantsCount`, у модификации нормализованные характеристики.
Цены, налоги, остатки и сырой ответ API не сохраняются. Полный проход каждого
типа получает собственный `MoySkladSyncCursor`/`MoySkladSyncRun`; отсутствие
записи в частичной выборке не удаляет её. Внешние модули пока не читают эти
таблицы напрямую; будущий доступ организуется через Facade.

`SyncCatalogAction` по сообщению `SyncCatalogMessage` (`async_sync`) сначала
выполняет полный проход товаров, затем модификаций, включая отдельные выборки
активных и архивных строк. Блокировка на подключение не допускает параллельных
проходов каталога. Каждая страница записывается транзакционно с upsert по
внешнему UUID; курсор соответствующего типа обновляется только после обеих
выборок. `MoySkladClient::fetchCatalogPage` проверяет форму страницы и
классифицирует ошибки, а `CatalogPageParser` проверяет аккаунт, архив,
ссылку товара и характеристики. `SyncCatalogHandler` повторяет 429 и временные
ошибки не более трёх раз; журнал и статус содержат только безопасную
категорию. Сортировка `id,asc` модификаций проверяется на каждой странице;
живого непустого ответа модификации пока нет, поэтому гарантия порядка API
...
Кнопка повторного запроса остаётся доступной при статусе `running`, чтобы
восстановить запуск после падения worker; advisory lock пропускает повтор,
если загрузка фактически ещё идёт.

Поток остатков хранит склады и неизменяемые снимки только в модуле MoySklad.
`MoySkladStore` имеет уникальность `(connection_id, external_id)` и содержит
идентичность, имя, коды, путь, архив и время источника/загрузки.
`MoySkladStockSnapshot` проходит состояния `building → completed|failed`;
читатели выбирают только последний `completed`. Каждая
`MoySkladStockSnapshotLine` ссылается на склад и ровно на один импортированный
товар или модификацию того же tenant и подключения через составные FK. Исходные
`stock`, `reserve`, `inTransit` хранятся как `numeric(30,10)` без вычислений.
Снимок публикуется только после полного прохода `/report/stock/bystore`; сбой
оставляет предыдущий completed снимок доступным. Записи Inventory не создаются.
Триггер БД разрешает вставку, обновление и удаление строк только пока снимок
