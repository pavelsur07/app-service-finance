...
- **В кроне стоит `--dry-run`.** Ночной прогон печатает, сколько записей и байт ушло бы, и ничего не удаляет. Переключение на `--execute` — решение Владельца на Production Gate: удаление необратимо и затрагивает production-аудит, а такое не должно включаться побочным эффектом выкатки стадии. Сначала настоящие цифры, потом решение.
