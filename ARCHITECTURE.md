...
- Проблемы разбора записываются ПАЧКОЙ (`recordMany`), а не по одной внутри цикла: поштучный вызов брал бы блокировку сырья и читал отметки на каждую проблему, а прогон заводит их до тысячи за раз и держит при этом блокировки всей пачки. Уникальные строки сырья блокируются одним запросом на компанию и в порядке идентификатора — тем же, что задаёт общий порядок блокировок.
