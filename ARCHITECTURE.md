...
- **Retention — год горячего хранения, удаляет приложение** (`app:ingestion:raw:prune`, cron `50 5 * * *`). Lifecycle-правило провайдера не годится: оно видит только объекты и снесло бы их, оставив строки `ingest_raw_records` висячими указателями — `ReadRawRecordAction` найдёт запись, пойдёт за объектом и упадёт. Удалять нужно ПАРУ, и знает про обе половины только приложение.
...
- Порядок удаления — строка, потом объект. Обратный даёт ровно тот висячий указатель, ради устранения которого всё и затевалось. При сбое между шагами остаётся осиротевший объект: он занимает место, но ничего не ломает, счётчик его показывает, путь уходит в лог, а команда возвращает ненулевой код. **Сам порядок доказан разбором отказов, а не тестом:** различает их только падение процесса между двумя шагами, которое в наборе не воспроизвести.
- Сырьё, на которое ссылается НЕРАЗОБРАННАЯ `NormalizationIssue`, не удаляется: оно и есть то, с чего начинает разбирающий. Удержание перепроверяется УЖЕ ПОД блокировкой строки сырья и отдельным запросом: `FOR UPDATE` защищает строку, но не отсутствие строки в таблице проблем, и условие внутри блокирующей выборки осталось бы снимком её собственного момента. **Гарантия ровно такая:** проблема, закоммиченная до перепроверки, всегда сохраняет своё доказательство. Проблема, закоммиченная между перепроверкой и коммитом удаления, доказательства лишится — закрыть это можно только общим протоколом блокировки для ВСЕХ, кто заводит проблемы (их шесть точек вне этой стадии), и это FOLLOW-UP. В production гонка сейчас не живая: крон запускает `--dry-run`. Состояние не вечное — закрытая проблема отпускает запись следующим прогоном, — но удержание считается отдельным счётчиком и попадает в `warning`: иначе «нечего удалять» и «удалять нельзя» выглядели бы одинаково.
