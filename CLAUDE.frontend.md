...
В проекте четыре слоя, каждый — единственный источник правды для своего:
...
**Исключение — behavior layer.** `ui-kit/components/states.css` легитимно принадлежит коду: там живёт поведение браузера и a11y (native pseudo-classes, autofill, focus), которого в монолите дизайнера нет и не будет. Правило из `states.css` — НЕ баг lint и НЕ регрессия при `uikit-update`. Новых классов behavior layer не вводит — только псевдоклассы на существующих классах UI Kit.
