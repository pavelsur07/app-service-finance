...
final class MoySkladStockSyncController extends AbstractController
{
    #[Route('/moy-sklad/connections/{id}/sync-stock', name: 'moysklad_connections_sync_stock', requirements: ['id' => Requirement::UUID], methods: ['POST'])]
    public function __invoke(string $id, Request $request, ActiveCompanyService $activeCompany, MoySkladConnectionWriteRepository $connections, MessageBusInterface $bus): Response
    {
        ...
        if (!$this->isCsrfTokenValid('moysklad_sync_stock'.$id, $request->request->getString('_token'))) {
            throw $this->createAccessDeniedException('Invalid CSRF token.');
        }
        if (!$connection->isActive() || ConnectionCheckStatus::CONNECTED !== $connection->getCheckStatus() || null === $connection->getAccountId()) {
            $this->addFlash('error', 'Проверьте и включите подключение перед загрузкой складов и остатков.');

            return $this->redirectToRoute('moysklad_connections_index');
        }

        $bus->dispatch(new SyncStockSnapshotMessage($companyId, $id));
        $this->addFlash('success', 'Загрузка складов и остатков поставлена в очередь. Если загрузка уже выполняется, повторный запрос будет пропущен.');
