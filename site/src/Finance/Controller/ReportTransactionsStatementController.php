...
class ReportTransactionsStatementController extends AbstractController
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        // Если область отчёта — Компания, и конкретный счёт не выбран,
        // исключаем внутренние перемещения/депозитные операции из расчёта движений.
        $excludeInternal = ('company' === $scope && null === $accountParam);
        ...
        if ($categoryParam && !array_filter($categoryOptions, static fn ($item) => $item['id'] === $categoryParam)) {
    ...
    private function fetchTransactions(
        ...
        foreach ($rows as $row) {
            ...
            // Названия/описания для фильтра
            $counterpartyName = $transaction->getCounterparty() ? $transaction->getCounterparty()->getName() : null;
            $description = $transaction->getDescription() ?? '';

            // Исключаем внутренние движения при company-scope без выбранного счёта:
            // переводы между своими счетами, вклад/депозит, возврат депозита и т.п.
            if ($excludeInternal) {
                $isOwnCounterparty = $counterpartyName && $counterpartyName === $company->getName();
                $isInternalByText = 1 === preg_match(
                    '/перевод.*между\s+счетами|вклад|депозит|возврат\s+депозит/iu',
                    $description
                );

                if ($isOwnCounterparty || $isInternalByText) {
                    continue;
                }
            }
     ...
     *   buckets: list<array{
     *     label: string,
     *     period: array{start: \DateTimeImmutable, end: \DateTimeImmutable},
     *     opening: float,
     *     closing_calc: float,
     *     closing_fact: float|null,
     *     check_ok: bool,
     *     check_diff: float|null,
     *     transactions: list<array{date: \DateTimeImmutable, doc: ?string, counterparty: ?string, description: ?string, amount: float, balance_after: float}>
     *   }>
     * }
     */
    private function aggregateByBuckets(
        ...
        // перенос расчётного сальдо между периодами
        $carryRunning = null;
        ...
        foreach ($periods as $p) {
            ...
            $start = $p['start'];
            /** @var \DateTimeImmutable $end */
            $end = $p['label_end'] ?? $p['end'];

            $startKey = $start->format('Y-m-d');
            $endKey = $end->format('Y-m-d');

            // Opening: перенос с прошлого периода или первый старт из balances
            if (null !== $carryRunning) {
                $opening = $carryRunning;
                $openingHasData = true;
            } else {
                $opening = 0.0;
            ...
            // Closing (факт по снапшотам) — только для сверки
            $closingFact = null;
            ...
            // Бежим по транзакциям периода последовательно
            $running = $opening;
            $periodTransactions = [];
            ...
            // переносим расчётное сальдо на следующий период
            $carryRunning = $closingCalc;
        }
