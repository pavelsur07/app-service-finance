...
final class IngestOrderStatusEventRepository extends ServiceEntityRepository
{
     ...
     * одновременно, и последовательность A → B → A → B вернулась бы в
     * произвольном порядке. Тай-брейк — `recordedSeq`, номер применения,
     * выданный счётчиком заказа под его блокировкой: он монотонен и между
     * процессами. `id` лишь замыкает сравнение для исторических строк без
     * номера. Ни сырьё, ни `occurrence` порядок здесь НЕ задают: первое
     * получает идентификатор задолго до разбора, второй считает наблюдения
     * внутри одного сырья и между разными несравним.
     *
