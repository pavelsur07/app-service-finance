...
final class IngestOrderRepository extends ServiceEntityRepository
{
    ...
    /**
     * Транзакционный advisory-замок на подключение: номер маркетплейса
     * присваивается и применяется ПО ОЧЕРЕДИ.
     *
     * Уникального ограничения на `external_order_id` нет, и быть не может —
     * коллизии есть в данных, их видят и откладывают. Но между запросом к
     * маркетплейсу и записью ответа проходят минуты, и за это время
     * нормализация могла присвоить тот же номер ДРУГОМУ заказу. Перепрос
     * проверял коллизии до сети и после неё видел только свой заказ: ответ,
     * ставший неоднозначным, применялся первому носителю — вплоть до
     * терминального статуса, после которого заказ навсегда выпадает из
     * перепроса.
     *
     * Замок берут обе стороны в начале своих транзакций: нормализация — до
     * блокировки сырья, перепрос — до блокировки заказов. Под ним коллизии
     * перепроверяются непосредственно перед записью, и проверка уже не
     * устаревает. Порядок «advisory → сырьё → заказы» один для всех.
     *
     * Ключ — hashtext от тройки: две компании с разными подключениями друг
     * друга не ждут.
     */
    public function lockConnectionScope(string $companyId, IngestSource $source, string $connectionRef): void
    {
        $this->getEntityManager()->getConnection()->executeStatement(
            'SELECT pg_advisory_xact_lock(hashtext(:scope))',
            ['scope' => sprintf('ingest_orders:%s:%s:%s', $companyId, $source->value, $connectionRef)],
        );
    }
