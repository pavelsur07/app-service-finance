 ...
 * строках одного отправления, а две строки с одним SKU и разными `offer_id` —
 * это разные предложения. Поэтому ключ — ПАРА `sku:<sku>|offer:<offerId>` плюс
 * номер повторения этой пары; он переживает и перестановку, и повтор.
 * `lineNo` остаётся порядком отображения.
 */
