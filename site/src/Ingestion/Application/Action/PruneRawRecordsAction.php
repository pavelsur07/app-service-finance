...
final readonly class PruneRawRecordsAction
{
    ...
    public function __invoke(PruneRawRecordsCommand $command): PruneRawRecordsResult
    {
        ...
        $result = new PruneRawRecordsResult(
            ...
            // ВЕРХНЯЯ ГРАНИЦА того, что освободит backlog.
            //
            // Удержанные записи вычтены — они места не отдадут заведомо. А вот
            // объекты, удалённые прошлым прогоном между `delete()` и падением
            // до коммита, отсюда не вычесть, не сходив в хранилище: это `limit`
            // сетевых вызовов на КАЖДЫЙ ночной dry-run ради поправки на
            // аварийное состояние. Прогноз честнее назвать границей, чем
            // оплачивать её уточнение обходом хранилища.
