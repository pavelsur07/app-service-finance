 ...
 * решение закоммичено, объект ещё на месте. `pendingBytes` — ВЕРХНЯЯ ГРАНИЦА
 * того, что backlog освободит: удержанные проблемой записи из неё вычтены —
 * они места не отдадут заведомо, — а объекты, удалённые прошлым прогоном между
 * `delete()` и падением до коммита, вычесть нельзя, не сходив в хранилище.
 * Уточнение стоило бы `limit` сетевых вызовов на каждый ночной dry-run ради
 * поправки на аварийное состояние; назвать число границей честнее. Она обслуживается ПЕРВОЙ и из
