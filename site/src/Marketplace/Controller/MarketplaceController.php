...
use App\Marketplace\Ozon\Infrastructure\Api\OzonCredentialValidationStatus;
...
class MarketplaceController extends AbstractController
{
    ...
    public function connectionStatus(string $id): JsonResponse
    {
        ...
        $status = match (true) {
            null !== $connection->getLastSyncError() => 'error',
            null !== $lastSyncAt => 'synced',
            default => 'pending',
        };
    ...
    public function syncRealization(string $id, Request $request): Response
    {
        ...
        $this->addFlash(
            ...
            sprintf(
                ...
                $count,
                1 === $count ? 'месяц' : ($count < 5 ? 'месяца' : 'месяцев'),
            ),
        );
    ...
    public function processCosts(string $id, Request $request): Response
    {
        ...
        if ('sales_report' !== $rawDoc->getDocumentType()) {
