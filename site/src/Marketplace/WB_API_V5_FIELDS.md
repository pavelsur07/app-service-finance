...
profitability = sku_result / net_sales
cost_coverage = covered_cost_quantity / (sold_quantity + returned_quantity)
return_rate_in_period = returned_quantity / sold_quantity
```
...
показываются как достоверные. Показатель возвратов относится только к
выбранному периоду: возврат может относиться к продаже из более раннего
периода, поэтому значение может превышать 100%.

База рентабельности — нетто-продажи без СПП за тот же выбранный период, а не
продажи с СПП и не сумма к перечислению. Если эта база меньше или равна нулю,
рентабельность не показывается.

HTML-блок и CSV-раздел «Товары и себестоимость» располагаются после
расшифровки удержаний и перед сводкой по `reportId`. В них отдельно видны:

- сопоставленные, несопоставленные и конфликтные SKU;
- проданное, возвращённое и нетто-количество;
- суммы без СПП, с СПП и товарное «К перечислению»;
- себестоимость продаж, возвратов и нетто-себестоимость;
- missing/fallback quantity и процент покрытия;
- результат до общих расходов WB либо явное отсутствие достоверного
  результата;
- корректировки `forPay`, которые не удалось распределить по SKU.
...
Canonical Ingestion применяет ту же формулу к `FinancialTransaction.direction`:
положительное удержание становится `OUT`, отрицательная выплата — `IN`.
Для известных оснований в `sourceData` сохраняются стабильный код, название и
группа категории; исходный `bonusTypeName` сохраняется без изменений.
