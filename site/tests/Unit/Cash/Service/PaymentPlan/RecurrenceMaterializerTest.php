...
namespace App\Tests\Unit\Cash\Service\PaymentPlan;
    ...
    public function testMaterializeCreatesMissingPlans(): void
    {
        ...
        $planRepository->expects($this->exactly(2))
            ...
            ->willReturnCallback(static function (
                ...
                PaymentRecurrenceRule $actualRule,
                \DateTimeInterface $date,
                string $amount,
                CashflowCategory $actualCategory,
            ) use ($company, $rule, $category, &$duplicateDates): bool {
                self::assertSame($company, $actualCompany);
                self::assertSame($rule, $actualRule);
                self::assertSame('100.00', $amount);
                self::assertSame($category, $actualCategory);
                $duplicateDates[] = $date->format('Y-m-d');

                return '2024-03-01' === $date->format('Y-m-d');
            });

        $recurrenceService = new PaymentRecurrenceService();
        $planService = new PaymentPlanService();
        ...
        self::assertSame(['2024-02-01', '2024-03-01'], $duplicateDates);
    }
