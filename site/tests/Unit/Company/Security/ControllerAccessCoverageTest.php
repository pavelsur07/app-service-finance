...
final class ControllerAccessCoverageTest extends TestCase
{
    public function testEveryControllerIsClassified(): void
    {
        ...
        // Факт на 2026-09-12: 180 routed-контроллеров (было 189 до удаления отчёта
        // «Сверка» — 9 контроллеров). Нижняя граница с запасом ~10% против «тихого»
        // зануления скана (например, при смене неймспейсов атрибутов).
        self::assertGreaterThan(160, $count, 'Контроллеры не найдены — проверьте скан src/.');
        self::assertSame(
            [],
            $unclassified,
            sprintf(
