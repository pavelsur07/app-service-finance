...
namespace App\Tests\Integration\Marketplace\Ozon\Infrastructure\Query;
...
use App\Marketplace\Ozon\Infrastructure\Query\OzonRawDuplicateAuditQuery;
...
final class OzonRawDuplicateAuditQueryTest extends IntegrationTestCase
{
    ...
    {
        $a = CompanyBuilder::aCompany()
            ->withIndex(1)
            ->withOwner(UserBuilder::aUser()->withIndex(1)->build())
            ->build();
        $b = CompanyBuilder::aCompany()
            ->withIndex(2)
            ->withOwner(UserBuilder::aUser()->withIndex(2)->build())
            ->build();
        $this->em->persist($a->getUser());
        $this->em->persist($b->getUser());
        $this->em->persist($a);
        $this->em->persist($b);
        ...
        $b2 = $this->raw($b, '2026-04-22', '2026-04-22', 6);
        foreach ([$a1, $a2, $a3, $b1, $b2] as $d) {
            $this->em->persist($d);
        }
        ...
        $doc = new Document(Uuid::uuid4()->toString(), $a);
        $this->em->persist($doc);
        ...
        foreach ([$s1, $s2, $s3, $s4, $s5, $r1, $r2, $c1, $c2] as $e) {
            $this->em->persist($e);
        }
        $this->em->flush();
        ...
        self::assertGreaterThan(1, (int) $sales[0]['raw_docs_count']);
        self::assertTrue((bool) $sales[0]['has_closed_rows']);
        ...
        self::assertTrue((bool) $returns[0]['has_closed_rows']);
        self::assertCount(1, $costs);
        self::assertTrue((bool) $costs[0]['has_closed_rows']);
    }

    public function testSingleDailyRawDocumentIsNotReportedAsOverlappingRawDocument(): void
    {
