...
use App\Marketplace\Wildberries\Application\FinancialReport\WbGeneratedRowsSafeReplaceService;
...
final class WbFinancialReportRefreshTest extends IntegrationTestCase
{
    public function testOpenSalesReturnsCostsRowsAreDeleted(): void
    {
        ...
        $this->em->persist($this->buildRawDocument($company, $rawDocId, $day));
    ...
    public function testLinkedRowsArePreservedWhileOpenRowsAreDeletedForPartialRefresh(): void
    {
        ...
        foreach ([$openSale, $linkedSale, $openReturn, $linkedReturn, $openCost, $linkedCost] as $e) {
            $this->em->persist($e);
        } $this->em->flush();
    ...
    private function buildRawDocument(Company $company, string $rawDocId, \DateTimeImmutable $day): \App\Marketplace\Entity\MarketplaceRawDocument
    {
