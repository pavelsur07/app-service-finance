...
final class OzonRawDuplicatesAuditCommandTest extends IntegrationTestCase
{
    public function testTableAndJsonAndInvalidDate(): void
    {
        ...
        $this->em->persist(MarketplaceRawDocumentBuilder::aDocument()->withIndex(2)->forCompany($company)->withMarketplace(MarketplaceType::OZON)->withDocumentType('sales_report')->withPeriod(new \DateTimeImmutable('2026-04-01'), new \DateTimeImmutable('2026-04-01'))->build());
        $this->em->flush();
        ...
        self::assertGreaterThanOrEqual(1, count($payload['overlapping_raw_documents']));
        self::assertArrayHasKey('company_id', $payload['overlapping_raw_documents'][0]);
        self::assertArrayHasKey('day', $payload['overlapping_raw_documents'][0]);
        self::assertArrayHasKey('range_from', $payload['overlapping_raw_documents'][0]);
        self::assertArrayHasKey('range_to', $payload['overlapping_raw_documents'][0]);
