...
16. **Behavior layer** (`components/states.css`, владелец — код). Контекст: у `components/*.css` два автора — дизайнер (через монолит + uikit-update) и инженер (a11y/поведение браузера: `:autofill`, `:disabled`, focus), модель источников правды второго не признавала. Решение: behavior layer — третий источник правды; живёт в `states.css`, подключается из `app.css` мимо `all.css` (каскадно последним), uikit-update его не регенерит. Отвергли exclusion-костыль «апдейт, не трогай этот import в all.css» — держится на памяти человека, однажды сломается. Платим: styling примитива в двух файлах (+1 место смотреть). Только токены, ноль хардкод-цветов, ноль новых классов.
    - **Known limitation (принято, не фиксим):** до первого пользовательского жеста Chrome/Chromium рендерит autofill-preview пароля собственным шрифтом/размером, игнорируя `font-size`/`font-family` страницы (маска точек крупнее/другого начертания, чем после клика) — задокументированный баг рендерера, открыт с 2019 (Chromium issue 960918, актуальные дубликаты 41471071/41457553). Это происходит на уровне рендерера, а не CSS-каскада — `!important`/specificity/transition-фризы (как в `states.css` для фона) на него не влияют. Единственный известный воркэраунд — `zoom` на `:-webkit-autofill`, который масштабирует весь box поля (нарушает фиксированную `height: 38px` DS) и создаёт свой flicker при коммите. Цена этого воркэраунда выше, чем у самого бага — видно доли секунды до первого клика/тапа, самоисправляется. Не пытаться фиксить снова без нового способа, не трогающего box-model.
...
- Старый `ui_theme=vf` читается только в течение окна совместимости одной версии.
- Переключатель и app-режим доступны только `ROLE_ADMIN`.
