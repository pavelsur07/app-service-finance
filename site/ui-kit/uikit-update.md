...
7. **Сверка прод-split с монолитом (ОБЯЗАТЕЛЬНО, защита от затирания прямых правок).**

   Прод-split мог опередить монолит: часть правок вносилась напрямую в split (модель скролла app-shell, `--s-8`, `states.css`), минуя монолит дизайнера. Разбор не должен их затереть.

   Проверить, что критичные прямые правки, живущие в прод-split, **присутствуют** в новом монолите. Если монолит их НЕ содержит — они будут затёрты разбором → 🛑 STOP.

   ```bash
   # Модель скролла app-shell — в проде есть, монолит должен её содержать
   echo "=== app-shell scroll model ==="
   grep -c "html:has\|body:has" site/ui-kit/patterns/app-shell.css   # prod split
   grep -c "html:has\|body:has" /tmp/storybook-new.html               # новый монолит
   # оба > 0, иначе монолит отстал → STOP

   # --s-8 — в проде используется
   echo "=== --s-8 ==="
   grep -c "\-\-s-8" site/ui-kit/tokens/spacing.css
   grep -c "\-\-s-8: 40px" /tmp/storybook-new.html
   # оба > 0

   # states.css — code-owned, в монолите его НЕТ и быть не должно (это норма)
   echo "=== states.css (code-owned, ожидаемо только в split) ==="
   test -f site/ui-kit/components/states.css && echo "prod: есть (owner=code, не трогать)"
   ```

   **Правило:** если новый монолит НЕ содержит правку, которая есть в прод-split (модель скролла, `--s-8`) → монолит отстал от прода. 🛑 STOP, вернуть дизайнеру на синхронизацию (задача `uikit-vX-sync-monolith.md`). НЕ разбирать — иначе прод откатится к старому состоянию.

   `states.css` — исключение: его в монолите нет намеренно (owner=code), это не повод для STOP.

8. Создать рабочую директорию для аудита:
   ```bash
...
- [ ] **Сверка прод-split ↔ монолит пройдена**: модель скролла app-shell и `--s-8` присутствуют в новом монолите (иначе STOP — монолит отстал)
...
разбирать монолит, отставший от прод-split           — STOP, вернуть дизайнеру на синхронизацию (иначе откат прода)
```
