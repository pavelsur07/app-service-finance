...
final class Version20260920110000 extends AbstractMigration
{
    ...
    {
        $this->addSql('CREATE UNIQUE INDEX uniq_moysklad_connections_company_id ON moysklad_connections (company_id, id)');
        $this->addSql('CREATE TABLE moysklad_counterparties (id UUID NOT NULL, company_id UUID NOT NULL, connection_id UUID NOT NULL, external_id UUID NOT NULL, name VARCHAR(255) NOT NULL, company_type VARCHAR(64) NOT NULL, legal_title VARCHAR(4096) DEFAULT NULL, inn VARCHAR(255) DEFAULT NULL, kpp VARCHAR(255) DEFAULT NULL, ogrn VARCHAR(255) DEFAULT NULL, ogrnip VARCHAR(255) DEFAULT NULL, legal_address VARCHAR(255) DEFAULT NULL, archived BOOLEAN NOT NULL, source_updated_at TIMESTAMP(3) WITHOUT TIME ZONE NOT NULL, loaded_at TIMESTAMP(3) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY(id))');
        ...
        $this->addSql('ALTER TABLE moysklad_counterparties ADD CONSTRAINT fk_moysklad_counterparties_connection FOREIGN KEY (company_id, connection_id) REFERENCES moysklad_connections (company_id, id) ON DELETE RESTRICT');

        $this->addSql('CREATE TABLE moysklad_sync_cursors (id UUID NOT NULL, company_id UUID NOT NULL, connection_id UUID NOT NULL, entity_type VARCHAR(64) NOT NULL, last_completed_at TIMESTAMP(3) WITHOUT TIME ZONE DEFAULT NULL, PRIMARY KEY(id))');
        ...
        $this->addSql('ALTER TABLE moysklad_sync_cursors ADD CONSTRAINT fk_moysklad_sync_cursors_connection FOREIGN KEY (company_id, connection_id) REFERENCES moysklad_connections (company_id, id) ON DELETE RESTRICT');

        $this->addSql("CREATE TABLE moysklad_sync_runs (id UUID NOT NULL, company_id UUID NOT NULL, connection_id UUID NOT NULL, entity_type VARCHAR(64) NOT NULL, status VARCHAR(16) NOT NULL, started_at TIMESTAMP(3) WITHOUT TIME ZONE NOT NULL, finished_at TIMESTAMP(3) WITHOUT TIME ZONE DEFAULT NULL, processed INT DEFAULT 0 NOT NULL, created INT DEFAULT 0 NOT NULL, updated INT DEFAULT 0 NOT NULL, unchanged INT DEFAULT 0 NOT NULL, error_category VARCHAR(32) DEFAULT NULL, PRIMARY KEY(id), CONSTRAINT chk_moysklad_runs_status CHECK (status IN ('running', 'succeeded', 'failed')), CONSTRAINT chk_moysklad_runs_counts CHECK (processed >= 0 AND created >= 0 AND updated >= 0 AND unchanged >= 0 AND processed = created + updated + unchanged), CONSTRAINT chk_moysklad_runs_outcome CHECK ((status = 'running' AND finished_at IS NULL AND error_category IS NULL) OR (status = 'succeeded' AND finished_at IS NOT NULL AND error_category IS NULL) OR (status = 'failed' AND finished_at IS NOT NULL AND error_category IS NOT NULL)))");
        ...
        $this->addSql('ALTER TABLE moysklad_sync_runs ADD CONSTRAINT fk_moysklad_sync_runs_connection FOREIGN KEY (company_id, connection_id) REFERENCES moysklad_connections (company_id, id) ON DELETE RESTRICT');
    ...
    {
        // Doctrine runs this migration transactionally. Lock before counting so
        // no worker can insert between the empty check and DROP TABLE.
        $this->connection->executeStatement('LOCK TABLE moysklad_counterparties, moysklad_sync_cursors, moysklad_sync_runs IN ACCESS EXCLUSIVE MODE');
        ...
        $this->addSql('DROP INDEX uniq_moysklad_connections_company_id');
    }
}
