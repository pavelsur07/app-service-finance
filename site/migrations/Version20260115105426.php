...
final class Version20260115105426 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_balance_cat_company_parent') THEN DROP INDEX idx_balance_cat_company_parent; END IF; END $$;");
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'uniq_balance_cat_company_code') THEN DROP INDEX uniq_balance_cat_company_code; END IF; END $$;");
        ...
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_balance_cat_company') THEN ALTER INDEX idx_balance_cat_company RENAME TO IDX_6250404A979B1AD6; END IF; END $$;");
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_balance_link_company_category') THEN DROP INDEX idx_balance_link_company_category; END IF; END $$;");
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'uniq_balance_link') THEN DROP INDEX uniq_balance_link; END IF; END $$;");
        $this->addSql('ALTER TABLE balance_category_links ALTER source_type TYPE VARCHAR(255)');
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_balance_link_company') THEN ALTER INDEX idx_balance_link_company RENAME TO IDX_D79DC3BA979B1AD6; END IF; END $$;");
        ...
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_constraint WHERE lower(conname) = lower('fk_doc_ops_project_direction')) THEN ALTER TABLE document_operations DROP CONSTRAINT fk_doc_ops_project_direction; END IF; END $$;");
        ...
        $this->addSql("DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'idx_pl_daily_company_cat_date') THEN DROP INDEX idx_pl_daily_company_cat_date; END IF; END $$;");
