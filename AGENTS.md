...
| `docs/workflow/release.md` | Migration deploy: dispatch, gates, request templates | PR carries a migration |
...
| `docs/workflow/data-migrations.md` | Data migrations: measurements, merges, post-deploy reconciliation | migration touching data |
| `docs/workflow/health-gates.md` | Monitors and gates: check coverage = fix coverage | cron check, verifier |
| `docs/workflow/new-module.md` | New module configuration and directory layout | creating `src/{Module}` |
| `docs/maintenance/quality-gates.md` | PHPStan baseline, cs-check, branch protection: design and history | red gate, CI trouble |
| `docs/maintenance/prod-access.md` | Production access: alias, wrappers, allowlists, log/backup paths | before touching production |
...
migration — read `docs/workflow/release.md` before the handoff request and ask
for both actions at once.
...
| **Large** | New module, schema or migration, financial logic, public API, Messenger/cron/workers, infrastructure, integrations, frontend redesign, cross-module change | Phase 0 → Stages → handoff | `plan.md`, `checkpoint.md`, `stages/stage-N.md`, `handoff.md` | §7.2: handoff + HIGH-LOCAL Stage with diff >500 lines |
...
- **Cascade.** Work item: the narrowest test plus focused lint/static analysis
  ...
  (§5). The full gates run once, at handoff — not at every Stage. On Fast
  Path/XS/Small the full suite runs only at handoff and only when integration
  paths are touched — running `make site-test` mid-task is a workflow
...
The final internal review at handoff runs in a **fresh session** holding only
the task spec, the complete diff and the checklist — not the implementation
history. Reviewing in the session that wrote the code anchors the reviewer on
its own decisions and misses its own mistakes.
...
- **When.** Fast Path and XS: never. Small: only when the change touches a
  ...
  Messenger routing) or the owner asks. Large: one round at handoff, plus one
  round at the end of a HIGH-LOCAL Stage whose diff exceeds ~500 lines — a
  diff too large to review well at handoff. Never after a Work item.
...
- Production is reached only through the `vf-prod-codex` alias and its wrappers
  ...
  rules, call forms and log/backup paths: `docs/maintenance/prod-access.md`.
  Read-only checks run when the owner asks or as post-deploy acceptance; every
  mutating production command is a §3.3 approval. Never work around a missing
  wrapper — ask the owner for a narrowly scoped one.
...
Long commands (full test suites, builds): launch once with an adequate timeout
or in the background and wait on the task result — never poll with manual
`docker ps`/`sleep` loops. Repeated targeted test runs: `docker compose exec`
into the running container, not `run --rm` — container startup dominates a
15–20 s test.
...
internal review result; external review result and rounds; review metrics —
BLOCKER/IMPORTANT counts found internally and externally (one line, feeds the
monthly review-policy check); commit, branch and PR URL; risks and follow-ups;
anything not completed; and at handoff the single §3.2 question. Be concise.
