...
Builder должен быть детерминированным: не использовать Faker, random-значения или случайные UUID. Он только создаёт объект и не обращается к БД, Symfony-контейнеру, HTTP-клиентам или внешним сервисам. Методы `with*()` валидируют аргументы, если для них существуют явные доменные ограничения.
...
### Источник истины для денежных сумм

Для денежных сумм использовать только существующий Value Object:
...
- сумму как целое количество минорных единиц (`amountMinor`);
- валюту как трёхбуквенный код в верхнем регистре (`currency`);
- точность валюты через `Symfony\Component\Intl\Currencies`;
- Doctrine-представление через `MoneyAmountType` и `#[ORM\Embeddable]`.

Нельзя создавать второй `Money`, собственный формат суммы или локальный набор денежных helper-функций внутри отдельного модуля. Расширение возможностей выполняется в существующем Value Object либо отдельным типом с другой семантикой.

### Обязательные правила

1. **Никогда не использовать `float` для денег, ставок, процентов и промежуточных финансовых расчётов.**
2. Денежная сумма всегда передаётся вместе с валютой. Даже нулевая сумма является `0 RUB`, `0 USD` и т. п.
3. Входные decimal-значения передаются строкой: `'123.45'`, а не `123.45`.
4. Форматированное значение (`'1 234,56 ₽'`) не хранится в БД и не участвует в расчётах.
5. Суммы разных валют нельзя складывать, вычитать или сравнивать без явной конвертации.
6. Итоги по нескольким валютам сначала группируются по валюте. Общий итог допустим только после конвертации по явно заданному курсу.
7. Направление операции (поступление/выбытие, доход/расход) не определяется классом `Money`. Это отдельное бизнес-свойство операции.
8. Исключения `MoneyMismatchException` и `MoneyOverflowException` нельзя подавлять, заменять нулём или пропуском строки без фиксации ошибки импорта.

### Создание Money

| Источник | Фабрика | Пример |
|---|---|---|
| Уже известны точные минорные единицы | `Money::fromMinor()` | `Money::fromMinor(12345, 'RUB')` → `123.45 RUB` |
| CSV/XLSX, форма, API, decimal из внешней системы | `Money::fromString()` | `Money::fromString('1 234,56', 'RUB')` |

```php
use App\Shared\Domain\ValueObject\Money;

$price = Money::fromString('1 234,56', 'rub');
$fee   = $price->percentage('15');
$total = $price->add(Money::fromMinor(10000, 'RUB'));
```

```php
// ❌ float уже потерял точность до попадания в Money
$calculated = 0.1 + 0.2;
$amount = Money::fromString((string) $calculated, 'RUB');

// ✅ decimal остаётся строкой на всей границе ввода
$amount = Money::fromString('0.30', 'RUB');
```

`fromString()` принимает точку или запятую как десятичный разделитель и удаляет обычные и неразрывные пробелы. Недопустимое значение должно завершаться ошибкой валидации, а не преобразовываться в ноль.

`fromMinor()` применять только тогда, когда источник действительно передаёт минорные единицы. Нельзя передавать в него рубли/доллары и компенсировать ошибку ручным умножением в Controller или Action.

### Арифметика и округление

Для арифметики использовать методы `Money`:
...
| Умножение | `multiply(string $factor, RoundingMode $mode)` |
| Процент от суммы | `percentage(string $percent, RoundingMode $mode)` |

Множитель и процент всегда передаются decimal-строкой:

```php
$commission = $revenue->percentage('17.5'); // 17,5%, не коэффициент 0,175
$quantityTotal = $unitPrice->multiply('3');
```

По умолчанию применяется `RoundingMode::HALF_UP`. Другой режим разрешён только когда он следует из конкретного бизнес-правила, договора или формата внешней системы. Выбранный режим должен быть явно передан и покрыт тестом.

Округление выполняется в определённой бизнес-правилом точке:

- если источник рассчитывает комиссию по каждой строке — округлять каждую строку;
- если правило задаёт процент от общего итога — сначала агрегировать, затем округлять;
- нельзя менять уровень округления ради совпадения итоговой суммы без описанного правила;
- нельзя повторно округлять уже полученный `Money` через строковое форматирование.

Ставка, процент, количество, курс валюты и коэффициент — **не Money**. Они хранятся и рассчитываются как отдельные точные decimal-значения. Если для них ещё нет утверждённого Value Object, нельзя временно использовать `float`: сначала согласовать тип и точность.

### Валюты и конвертация

`Money` разрешает арифметику только в одной валюте. `MoneyMismatchException` означает ошибку бизнес-сценария, а не необходимость автоматически выбрать курс.

Конвертация валют должна выполняться отдельным сервисом и содержать минимум:

- исходную и целевую валюту;
- decimal-курс без `float`;
- дату/время курса;
- источник курса;
- направление котировки;
- правило округления результата.

Исходная сумма, курс и сконвертированный результат должны сохраняться или быть воспроизводимыми для аудита. `Money::multiply()` само по себе не является операцией конвертации валюты, потому что сохраняет исходный код валюты.

### Поддерживаемая точность валют

Для известных валют количество знаков определяется через `Currencies::getFractionDigits()`:

| Валюта | Минорных знаков | Пример |
|---|---:|---|
| RUB, USD, EUR | 2 | `12345` → `123.45` |
| JPY | 0 | `12345` → `12345` |
| BHD | 3 | `12345` → `12.345` |

Текущая реализация технически использует масштаб `2`, если код отсутствует в Intl. Это fallback совместимости, **не бизнес-правило**. Новый код не должен считать любую строку из трёх букв поддерживаемой валютой только потому, что `Money` принял её.

До добавления отдельного каталога валют:

- для новых фиатных валют проверить поддержку кода и scale в Symfony Intl;
- неизвестную валюту не импортировать молча с двумя знаками;
- зафиксировать ошибку импорта и исходное значение для повторной обработки.

### Криптовалюты

Текущий `Money` нельзя автоматически применять для криптовалют:

- Symfony Intl может не знать требуемый scale;
- fallback `2` неверен для BTC, ETH, USDT и других активов;
- доступного диапазона PHP `int` может быть недостаточно при большом scale;
- трёхбуквенного кода недостаточно для описания сети и конкретного актива.

До утверждения отдельного решения криптовалютные суммы нельзя округлять до двух знаков или хранить через `float`. Нужен отдельный паттерн: каталог активов/сетей, scale, безопасный тип хранения и правила конвертации. Изменять `Money::fractionDigits()` точечно под одну криптовалюту запрещено.

### Doctrine и БД

В Entity денежное значение встраивается как `Money`:
...
- использовать существующий `MoneyAmountType`, не `FLOAT`, `REAL` или `DOUBLE PRECISION`;
- не дублировать рядом отдельные изменяемые поля `amount` и `currency`;
- обязательная сумма должна иметь заполненные amount и currency;
- для необязательной суммы оба значения должны быть одновременно `NULL` либо одновременно заполнены;
- SQL-агрегации выполняются по `amountMinor` и обязательно группируются/фильтруются по валюте;
- результат `SUM` проверяется на допустимый диапазон перед созданием `Money`;
- фактический SQL-тип, диапазон и преобразование определяет `MoneyAmountType` — локальное Doctrine-маппирование не копируется.

Сырой decimal из внешней системы можно сохранить в raw-документе для аудита, но рабочая денежная сумма создаётся через `Money` на этапе нормализации.

### API, Messenger и UI

В публичном JSON сумма передаётся decimal-строкой, чтобы JavaScript не потерял точность:
...
Нельзя отдавать денежное значение JSON-числом:

```json
{ "amount": 1234.56 }
```

Для очередей и межмодульных контрактов сумма и валюта передаются явно. Если контракт может быть прочитан не-PHP клиентом, минорные единицы также сериализуются строкой, чтобы избежать ограничения JavaScript `Number`.

`toDecimalString()` используется для канонического машинного представления. Локализованное отображение (`1 234,56 ₽`) выполняется отдельным formatter-сервисом только в Presentation-слое.

### Знак суммы

`Money` поддерживает положительные и отрицательные значения, но не определяет их бизнес-смысл. Каждый модуль должен иметь одну зафиксированную конвенцию:

- либо знак хранится в сумме;
- либо направление хранится отдельным Enum/флагом, а сумма остаётся неотрицательной.

Нельзя одновременно менять знак суммы и инвертировать направление операции. Конвенция должна быть одинаковой в импорте, Entity, отчёте и API и покрываться интеграционным тестом.

### Обязательные тесты

- `fromMinor()` и `fromString()` для RUB, JPY и валюты с тремя знаками;
- точка, запятая, пробел, неразрывный пробел, отрицательное значение;
- границы округления для положительных и отрицательных сумм;
- `add()`, `subtract()`, `multiply()`, `percentage()`, `negate()` и `abs()`;
- запрет арифметики разных валют;
- ноль в разных валютах;
- `PHP_INT_MAX`, `PHP_INT_MIN` и переполнение каждой арифметической операции;
- round-trip `fromString()` → `toDecimalString()`;
- Doctrine persist/hydrate round-trip;
- API-сериализация decimal-строкой;
- SQL-агрегация только внутри одной валюты;
- конвенция знака для каждого финансового модуля.

### Известные ограничения текущего Money

Codex не должен обходить эти ограничения локальным кодом. Для изменения требуется отдельная задача на сам Value Object и его тесты.

1. Неизвестная Intl валюта получает fallback scale `2`.
2. `add()`, `subtract()` и `negate()` не используют `createSafe()` и требуют отдельной проверки единообразной обработки overflow.
3. Для `multiply()` и `percentage()` промежуточный scale BCMath зафиксирован на `12`.
4. Нет операций деления, распределения остатка и конвертации валют.
5. Не определена модель точности и хранения криптовалют.
...
// ❌ разные валюты агрегируются в один итог
$total = $rub->add($usd);

// ❌ курс валюты представлен как Money
$rate = Money::fromString('92.50', 'RUB');

// ❌ Presentation-строка сохраняется в БД
$entity->setAmount('1 234,56 ₽');
```
