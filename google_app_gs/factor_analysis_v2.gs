...
const FACTOR_ANALYSIS_CONFIG = Object.freeze({
  ...
  metricAliases: {
    ...
    grossProfit: [
      'Маржинальная прибыль CM1',
      'Маржинальная прибыль 1 (CM1)',
      'CM1 (маржинальная прибыль 1)',
      'Валовая прибыль'
    ],
    promoCosts: ['Расходы на продвижение'],
    cm2: [
      'Маржинальная прибыль 2 (CM2)',
      'Маржинальная прибыль CM2',
      'CM2 (маржинальная прибыль 2)'
    ],
    marketplaceOpex: [
      'Операционные расходы склад / прочие расходы МП',
      'Операционные расходы склад / прочие МП',
      'Операц. расходы склад / прочие МП',
      'OPEX склад и пр. МП'
    ],
    cm3: [
      'Маржинальная прибыль 3 (CM3)',
      'Маржинальная прибыль CM3',
      'CM3 (маржинальная прибыль 3)'
    ],
...
function getFactorAnalysisSourceData_(ss) {
  ...
  const availableMonths = cfg.monthOrder.map(function (_, index) {
    return hasFactorAnalysisMonthData_([
      ...
      metrics.variableCosts.values[index],
      metrics.promoCosts.values[index],
      metrics.marketplaceOpex.values[index],
      metrics.overheadCosts.values[index],
      metrics.belowEbitdaExpenses.values[index],
      metrics.belowEbitdaIncome.values[index]
    ]);
  });

  const factorAnalysisMonths = cfg.monthOrder.map(function (_, index) {
    return Math.abs(toFactorAnalysisNumber_(metrics.revenue.values[index])) > 0.000001;
  });
  ...
  return {
    ...
    factorAnalysisMonths: factorAnalysisMonths,
    warnings: warnings
  };
}
...
function buildFactorAnalysisRows_(sourceData) {
  ...
  pushRow([
    'Факторное разложение EBITDA выполняется только для соседних месяцев, где выручка обоих месяцев не равна нулю.'
  ], 'note');
  pushRow(['']);
...
function buildFactorAnalysisGeneralSection_(
  ...
  for (let i = 1; i < sourceData.months.length; i++) {
    ...
    if (!sourceData.factorAnalysisMonths[i - 1] || !sourceData.factorAnalysisMonths[i]) {
      pushRow([
        comparison,
        '', '', '', '', '', '', '', '', '', '', '', '',
        'нет выручки для факторного анализа'
      ]);
...
function buildFactorAnalysisMarginFactorsSection_(
  ...
  for (let i = 1; i < sourceData.months.length; i++) {
    ...
    if (!effectInfo || !effectInfo.hasData) {
      detailRowCount++;
      pushRow([comparison, '', '', '', '', '', '', 'нет выручки для факторного анализа']);
      continue;
    }
