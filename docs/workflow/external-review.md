...
получает раунд на handoff только когда изменение затрагивает HIGH-LOCAL зону
(миграция, auth, финансовая семантика, публичный endpoint, Messenger routing)
или Владелец попросил явно; Large — один раунд на handoff. Повтор раунда нужен
только после исправленного BLOCKER. Больше трёх раундов на одну точку ревью —
STOP с отчётом, а не четвёртый раунд.
...
`--effort medium` — по умолчанию для всех раундов.
