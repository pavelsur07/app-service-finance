...
- Отдельный раунд Stage 3 не требуется: diff <500 lines. Финальный handoff round 1: BLOCKER 0; заявленный IMPORTANT о длине variant.name отклонён, поскольку закреплённая официальная схема определяет name и значения характеристик как String(255). Из двух MINOR добавлен прямой tenant-negative тест репозиториев; смена доверенного host отвергнута, поскольку ссылка родителя сознательно принимается только с официального api.moysklad.ru, а custom base URL используется для транспортных тестов.
