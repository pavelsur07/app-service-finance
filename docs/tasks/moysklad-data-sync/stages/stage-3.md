...
- После неудачного ORM flush исход страницы откатывается, а `failed/internal` фиксируется через DBAL, который остаётся доступен после закрытия EntityManager.
...
- [x] Проверены рестарт с `offset=0`, отсутствие удаления при пустом проходе, 401/403/неверный JSON после успешного cursor и ошибка хранения страницы.
...
- После handoff-review добавлены сценарии рестарта и ошибок; исчерпание повторов пишет агрегированный `error` без ответа/секрета.
