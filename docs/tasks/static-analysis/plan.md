...
release_candidate: no
independently_deployable: yes
stage_base_commit: `53f31e2588144a20876ff5f6fc03610e5a53f5c8`

Обоснование риска: одна dev-зависимость, один новый файл правил в `tests/` и
регистрация сервиса в конфиге анализатора. Ни рантайм-кода, ни CI-логики,
ни схемы БД. Цепочка деплоя не затрагивается вовсе.

Смысл Stage: половина запретов `CLAUDE.md` сегодня держится только на глазах
ревьюера. PHPat превращает их в правила PHPStan — тот же прогон, тот же baseline,
тот же гейт, без второго инструмента и второй конфигурации.

### Определение области правила — по замеру, а не по лозунгу
Первый вариант «снаружи модуля виден только Facade» отвергнут: Facade этого
проекта возвращают Entity и Enum, поэтому легальное использование Facade
требует импортировать эти типы, и «хорошее состояние» оказалось бы недостижимым.
Распределение 781 внешнего импорта в модули с Facade:
...
Цена вариантов: «только Facade» — 703 нарушения, «Facade+DTO+Enum» — 562,
«Facade+DTO+Enum+Entity» — 230, дословный запретный список `CLAUDE.md`
(`Service/`, `Repository/`, `Application/`, `Infrastructure/`) — 122.
Выбран последний: он опирается на уже написанный запретный список проекта, и у
каждого нарушения есть понятная операция починки. 122 — предварительный замер по
сырому списку; в реализации из закрытого `Application/` осознанно исключён
публичный подслой `Application/DTO` (иначе типизированный вызов Facade сам стал
бы нарушением), поэтому фактический результат PHPat — 118. Исключение внесено
в нормативный раздел `CLAUDE.md`, чтобы правило и гейт не расходились.
Ужесточение — FOLLOW-UP.

### Definition of Done
- `phpat/phpat` установлен, правила зарегистрированы тегом `phpat.test`.
- Правило закрытых слоёв действует для всех модулей, объявивших Facade **классом**, а не пустым каталогом.
- Правило заморозки легаси-зоны действует и зелёное.
- Модули без Facade в правило не включены: у их нарушений не было бы
  легального способа починки.
- Гейт остаётся зелёным и доказан красным отдельно для каждого из двух правил.
- Дельта baseline зафиксирована с разбивкой по модулям.
- Влияние на время прогона измерено, а не оценено на глаз.

### Явно вне scope Stage 3
- Исправление найденных нарушений границ (по факту — 118).
- Ужесточение правила до Facade+DTO+Enum+Entity.
- Правило «`#[ManyToOne]` на чужую Entity» — PHPat не разбирает аргументы
  атрибутов на уровне свойств.
- Детектор циклов между модулями — в PHPat его нет, нужен deptrac.
- Собственные правила PHPStan и `spaze/phpstan-disallowed-calls` (Stage 4).

### Work items
- 3.1 — установить `phpat/phpat`.
- 3.2 — `tests/Architecture/ModuleBoundaryRules.php`.
- 3.3 — регистрация в `phpstan.dist.neon`, `show_rule_names`.
- 3.4 — перегенерация baseline, замер дельты по модулям.
- 3.5 — негативные пробы обоих правил и замер производительности.

### Stage checks
- `composer stan` — exit 0.
- Две негативные пробы — ненулевой exit с именем сработавшего правила.
- Тёплый и холодный прогон — сравнение с Stage 2.
- `composer cs:check`, `composer cs:strict-types`, `composer test:unit`.
- `tests/Architecture` не подхватывается ни одним testsuite PHPUnit.

### Reviewer focus
- Селекторы: нет ли ложных срабатываний и пропусков по построению.
- Не выхолощено ли правило выбором запретного списка.
- Честность описания остаточного пробела правила легаси-зоны.

## Stage 4: ratchet — запрещённые вызовы, правило `companyId`, запрет роста baseline
Risk: MEDIUM
owner_gate: yes
release_candidate: no
independently_deployable: yes
stage_base_commit: `cf1ef467cda672362086e7738ba8deb822485346`

Обоснование риска: dev-зависимость, одно собственное правило PHPStan в `tests/`
и один дополнительный шаг в CI-job `stan`. Рантайм-код, схема БД и цепочка
деплоя не затрагиваются; job `stan` по-прежнему не входит ни в один `needs`
цепочки `build-and-push → schema-ready → deploy`.

Смысл Stage: замкнуть ratchet. Stage 1-3 сделали гейт зелёным и наполнили его
правилами, но baseline пока сдерживается только договорённостью «только
сокращать». Плюс два запрета `CLAUDE.md`, которые до сих пор проверялись глазами:
отладочные вызовы и `companyId` в Repository — последний прямо назван в
`CLAUDE.md` первым пунктом ревью, потому что «IDOR в проде = инцидент».

### Замеры до реализации
- Отладочные вызовы в `src`+`tests`: `dump(` 0, `dd(` 0, `var_dump(` 0,
  `print_r(` 0, `die(` 0, `exit(` 0. Единственные `var_export()` — два вызова
  в `tests/Unit/Marketplace/Domain/Backward/OzonResolveBackwardCompatTest.php`
  для текста сообщения об ошибке, они легитимны. Значит гейт запрещённых
  вызовов зелёный с первого дня — чистый ratchet без долга.
- Публичные методы Repository: 512, из них 386 уже принимают `$company`/`$companyId`.
  Из 406 методов-запросов (`find*`, `get*`, `count*`, `sum*`, …) grep дал
  **76 без параметра компании**. Итоговое число после шести итераций внешнего
  ревью — **51**; промежуточные 76 → 71 → 35 → 25 → 39 сохранены в Stage Report
  как исторические замеры вместе с причиной каждой поправки.

Правило намеренно ограничено методами-запросами: `save()` (29 штук) и `remove()`
компании не требуют — сущность её уже несёт. Требовать `companyId` от них
значило бы пометить плохим состояние, из которого нет разумного выхода.

### Definition of Done
- `spaze/phpstan-disallowed-calls` настроен на запреты `CLAUDE.md`; гейт зелёный
  без единой записи в baseline.
- Собственное правило проверяет `companyId` в методах-запросах Repository,
  покрыто unit-тестом либо, если `RuleTestCase` в этой конфигурации недоступен,
  доказано пробами с записью ограничения в отчёт.
- Существующие нарушения уходят в baseline; для каждого понятен путь —
  добавить параметр либо оформить явный отказ с причиной.
- В CI появляется проверка, запрещающая рост baseline относительно базы PR,
  с явным и документированным способом провести осознанный рост.
- Гейт остаётся зелёным и доказан красным для каждого нового правила отдельно.

### Явно вне scope Stage 4
- Исправление найденных нарушений `companyId` и 118 нарушений границ.
- Call-site правило для унаследованных finder'ов Doctrine — отдельная задача.
- Сжатие baseline.
- Оба найденных бага (`$roleId`, `$this->_em`) — отдельные задачи с
  регрессионными тестами.

### Work items
- 4.1 — `spaze/phpstan-disallowed-calls` и конфигурация запретов.
- 4.2 — собственное правило `RepositoryCompanyScopeRule` и его тест.
- 4.3 — проверка запрета роста baseline в CI-job `stan`.
- 4.4 — перегенерация baseline, пробы, замеры.

### Stage checks
- `composer stan` — exit 0.
- Проба на `dump()` — ненулевой exit.
- Проба на query-метод Repository без `companyId` — ненулевой exit.
- Проверка роста baseline: доказать, что она краснеет на искусственном росте.
- `composer cs:check`, `composer cs:strict-types`, `composer test:unit`.

### Reviewer focus
- Правило `companyId` не помечает недостижимых состояний (`save`/`remove`).
- Escape для осознанного роста baseline существует, документирован и не
  превращается в дыру.
- Проверка роста работает на shallow-checkout CI.
