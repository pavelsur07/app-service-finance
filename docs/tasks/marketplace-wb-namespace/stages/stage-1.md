...
#### CI: baseline-guard и `[baseline-grow]`
- `site/bin/phpstan-baseline-guard.sh` сравнивает записи по путям и не понимает переименований: 248 записей перенесённых файлов посчитаны «новыми».
- Проверено отдельно: baseline ветки совпадает с baseline `master` (`f4550ada`) после применения карты переименований — все 2410 записей, включая 3 записи вспомогательных классов в перенесённых тестах. Роста нет; в заголовок PR добавлена штатная метка `[baseline-grow]` с обоснованием в описании.
- FOLLOW-UP: научить guard учитывать `git diff -M` переименования, чтобы этап 6 (Ozon) прошёл без метки.
