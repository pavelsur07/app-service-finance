...
#### Раунд 2: `REVIEW_GREEN`, разбор оставшихся MINOR

Прогон 3 (`--max-turns 80`) подтвердил все четыре исправления и завершился
`REVIEW_GREEN` — BLOCKER/IMPORTANT нет. Оставшиеся замечания:

| # | Finding | Вердикт | Действие |
|---|---|---|---|
| MINOR M1 | Инвариант «ИП + 10-значный ИНН» чинился только в `SaveCounterpartyAction`, поэтому импорт 1С сохранял несогласованную подсказку до следующего backfill | подтверждён | Сброс перенесён в `assignTaxIds()` — инвариант держится во всех путях записи. `clearLegalFormHint()` удалён как ненужный, Action логирует `warning`, наблюдая факт сброса (предикат не дублируется). Два unit-теста: авторемонт и переходное состояние между `rename()` и `assignTaxIds()` |
| MINOR M2 | Экранирование `%`/`_` без регрессионного теста | подтверждён | Два integration-теста: `р%` и `р_` не вытаскивают подстрочные совпадения |
| MINOR M3 | `ARCHITECTURE.md` и `stage-2.md` описывали старое имя опции, `effectiveLegalFormHint()` не задокументирован | подтверждён | Документация обновлена |
| MINOR M4 | В импорте ключ кэша приводится к нижнему регистру, а поиск в БД регистрозависимый | подтверждён как pre-existing | Точечно не правим: правильное решение — переход на `nameCore` (регистронезависим по построению) в Stage 4. Зафиксировано в `checkpoint.md` |
| FOLLOW-UP F1 | После релиза исторические строки с двойными пробелами дадут по одной новой записи при первом импорте | принят | Ожидаемый одноразовый эффект описан в порядке деплоя в `checkpoint.md` |
| FOLLOW-UP F2 | В рабочем дереве лежат посторонние untracked-артефакты, включая `reports/*.csv` с реальными данными ДДС | принят | В коммиты задачи не попали; удалять чужие файлы не стал — это решение Владельца |
