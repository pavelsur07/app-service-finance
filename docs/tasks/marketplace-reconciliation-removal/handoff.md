...
строгий: выгрузить `stored_file_path` → сверить, что ни один путь не встречается в
`MarketplaceMonthClose.settings->'costsReconciliation'->>'file_path'` → удалить
объекты поштучно → только потом дроп таблицы. После дропа список взять негде.
