...
1. Выполнить read-only probe:
   `app:ingestion:ozon-accrual:probe --endpoint=types --with-values`.
2. Сравнить `type_id`/названия Ozon с `OzonAccrualCategory::all()`.
3. Добавить новый `type_id` в существующую строку или создать новую строку.
4. Добавить/обновить unit-тест справочника и mapper-а.
