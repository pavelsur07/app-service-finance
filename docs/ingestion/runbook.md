...
Пусто → это перетипизация, а не stale. Тогда искать остаток по типу, который
перестал появляться. Обнулённые строки из выдачи убирать обязательно: они
сохраняют прежний тип, и без фильтра уже починенные дни выглядят грязными.

```sql
SELECT shop_ref, min(d) AS first_dirty, max(d) AS last_dirty,
       count(*) AS dirty_days, sum(o) AS live_other
FROM (
  SELECT shop_ref, DATE(occurred_at) d,
         count(*) FILTER (
           WHERE type='other'
...
) t WHERE o > 0 GROUP BY 1 ORDER BY 1;
```

Остаток не обязан быть непрерывным до даты деплоя: дни внутри 45-дневного окна
могли починиться сами (см. ниже), поэтому опираться нужно на эту выдачу, а не на
арифметику от даты релиза.
...
Реплей пишет **только те дни, для которых снапшот является авторитетом**
(`selected_dates` из `latestCoverageRows`). Это важная гарантия: снапшот
покрывает несколько дней, но владеет лишь частью, и без ограничения на чужих днях
рядом с актуальной строкой появлялась вторая с другим типом — суммы удваивались,
а prune их не убирал, потому что сносит перекрытых, а не перекрывающих. Строки с
неразборчивой датой ограничение пропускает дальше, чтобы испорченное сырьё
по-прежнему падало с issue, а не исчезало молча.

Ограничение распространяется на весь путь разбора — запись, гашение
(`OzonAccrualStaleComponentVoider`) и удаление (`OzonAccrualStaleProjectionPruner`)
работают по одному набору дней. Это не косметика: гасилка убирает строки того же
сырья, которых нет в разобранном наборе, и при суженном наборе без такого же
ограничения она обнуляет дни за пределами окна. Так 18.09.2026 на проде погасли
19 дней, 26 496 строк — восстановлено реплеем с окном, равным полному владению
снапшота.
...
**Когда перетипизация чинится сама, а когда нет.** Ночной `rolling-refresh`
берёт все 45 дней и завершается — глубина у него та же, что у сверки. Но
перенормализация происходит не всегда: дедуп сравнивает `external_id` **и хеш
содержимого**, и если Ozon отдал те же данные, запись переиспользуется
(`markSeen()`), а старая типизация остаётся.

Отсюда выводы для разбора:

- если Ozon внутри 45-дневного окна правит начисления задним числом, хеш чанка
  меняется, появляется новая запись сырья, она нормализуется текущим маппером — и
  дни этого чанка перетипируются сами, без обнулённых строк (прунер сносит строки
  перекрытого снапшота целиком);
- день, вышедший из 45-дневного окна, не перевыбирается больше никогда, и его
  типизация остаётся такой, какой была на момент последней нормализации.

Поэтому после правки маппера остаток всегда один и тот же: дни, которые вышли из
окна раньше, чем Ozon успел их поправить. Их и добирают `normalize-stored`.
Границу остатка искать запросом про живые строки старого типа (см. выше), а не
по дате деплоя: часть дней внутри окна чинится сама и попадает в выдачу как уже
чистая.
