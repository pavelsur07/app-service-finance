...
> **Режим работы — автономный.** Иерархия выполнения: `Work item → Stage → Handoff → merge + deploy`. Владельцу задаётся один вопрос за задачу — «merge and deploy» на handoff. Work item завершается таргетированными проверками и focused self-review; Stage — полным review, Stage Report и внешним ревью там, где его требует `AGENTS.md` §7.2. Локальный риск усиливает проверки, но не создаёт ручную остановку.
...
| `docs/workflow/external-review.md` | Внешнее ревью: команды, промпт, бюджет раундов | Перед вызовом второго агента |
| `docs/workflow/templates.md` | Шаблоны plan, checkpoint, Stage Report, handoff, STOP | Large-задача |
| `docs/workflow/git-housekeeping.md` | База PR, удаление веток | Перед `gh pr create` и удалением ветки |
| `docs/workflow/new-module.md` | Конфигурация нового модуля | При создании `src/{Module}` |
| `docs/maintenance/prod-access.md` | PROD-доступ: правила, wrappers, allowlist, формы вызова | До любого обращения к production |
...
Handoff → merge + deploy`, единственный вопрос Владельцу («merge and deploy»),
pre-authorized действия, полный список STOP-условий, классификация задач
(Fast Path / Small / Large) и рисков, верификация, политика внутреннего и
внешнего ревью, git/PR, безопасность и полный список запретов.
...
Реализующий агент сам вызывает **другого** агента как read-only ревьюера: Codex
ревьюит Claude Code, Claude Code ревьюит Codex. Когда ревью обязательно, что
считается зелёным и бюджет раундов — `AGENTS.md` §7.2. Команды, стандартный
промпт, stdin-форма для песочницы и обработка сбоев —
`docs/workflow/external-review.md`.

Коротко: Fast Path — без внешнего ревью; Small — один раунд на handoff; Large —
раунд на каждый HIGH-LOCAL Stage и один на handoff. Повтор раунда только после
исправленного BLOCKER; исправленные IMPORTANT подтверждаются проверками и
внутренним review без повторного вызова. Не больше трёх раундов на точку.

### Закрытие Stage — репозиторный checklist

Порядок закрытия — `AGENTS.md` §5; шаблоны Stage Report и handoff —
`docs/workflow/templates.md`; база PR и удаление веток —
`docs/workflow/git-housekeeping.md`. Специфичный для этого репозитория self-review
...
**Красный baseline — не регресс, но и не индульгенция.** Репозиторная проверка может оказаться красной до задачи. В этом случае проверять точечно только изменённые файлы, сравнивать с их состоянием до правки и записывать в отчёт факт красного baseline с цифрой. Не выдавать чужие нарушения за свои и не прятать за ними свои. Проверено 2026-08-30: `make site-cs-check` и `make site-cs-strict-types` сейчас **зелёные** (`Found 0 of 2363`, exit 0; cs-check перепроверен с `--using-cache=no` — тёплый кэш даёт ложную зелень). Статический анализ — `make site-stan` (PHPStan, level 8). Он достижимо зелёный: унаследованные ошибки зафиксированы в `site/phpstan-baseline.neon` (3689: 118 нарушений границ модулей и 51 метод-запрос Repository без `companyId`), анализируется 2347 из 2365 файлов `src`+`tests`; исключения перечислены в `site/phpstan.dist.neon`. Расширения читают контейнер из `var/cache/test`, поэтому `make site-stan` сначала прогревает его сам. Границы модулей проверяются машинно: правила в `site/tests/Architecture/ModuleBoundaryRules.php` запрещают зависеть от `Service/`, `Repository/`, `Application/` (кроме `Application/DTO`), `Infrastructure/` чужого модуля и держат легаси-зону пустой. Отладочные вызовы (`dump`, `dd`, `var_dump`, `print_r`, `die`, `exit`) запрещены `spaze/phpstan-disallowed-calls` — записей в baseline нет, гейт чистый. Рост baseline блокируется в CI (`site/bin/phpstan-baseline-guard.sh`); осознанный рост проводится меткой `[baseline-grow]` в заголовке PR. Baseline только сокращать — перегенерация «начисто» (`make site-stan-baseline`) стирает ratchet и делает регресс неотличимым от долга. Таргета `make stan` без префикса в проекте нет.

**Обязательные проверки `master` заданы в защите ветки, а не в коде.** На `master`
включена branch protection. Обязательны три проверки: `🔬 Static analysis`,
`🎨 PHP code style`, `🧪 Unit tests`. Красная проверка блокирует merge.
Integration- и functional-тесты остаются советующими — почему, ниже.

**Имя job'а в `.github/workflows/deploy.yml` — это контракт с защитой ветки.**
Переименовал job — обязательная проверка со старым именем больше не появится
никогда, и PR встанет навсегда: удовлетворить её нечем. Список обязательных
проверок обновляется в том же PR, что меняет имена.

**Матричный job обязательным сделать нельзя.** Это проверено, а не предположено:
при попытке потребовать `🧪 Integration tests` и три `🧪 Functional tests N/3`
docs-only PR #2388 заблокировался навсегда. Причина — когда матричный job
пропускается по `if:`, GitHub публикует один check с **нераскрытым шаблоном**
`🧪 ${{ matrix.name }}`, а не три раскрытых имени. Требуемые контексты не
появляются вовсе и остаются `pending`. Чтобы сделать эти тесты обязательными,
матрицу нужно сначала развернуть в отдельные job'ы со статическими именами.

Пропуск обычного, не матричного job'а безопасен: workflow запускается на каждый
PR (`paths`-фильтра на уровне `on:` нет), поэтому check появляется всегда, а
пропущенный GitHub засчитывает как пройденный. Именно поэтому фронтенд-only и
docs-only PR не блокируются тремя обязательными проверками выше.

Защита не распространяется на админов — аварийный обход остаётся, но пользоваться
им вместо починки означает ровно то же, что перегенерировать baseline вместо
исправления ошибки.
...
**Миграции данных.** Схему менять по expand/contract: добавить совместимое → перенести данные отдельным шагом → переключить код → удалить старое отдельной задачей; не совмещать все четыре в одном Stage без явного требования. Список дочерних таблиц брать из реальной схемы (`information_schema`, `pg_constraint`), а не из памяти или прошлой миграции. Для каждого уникального ключа, содержащего переносимую колонку, заранее решить судьбу конфликта. Денежные строки при слиянии суммировать, а не удалять. Порядок шагов проверять на частичных уникальных индексах: перенос идентичности возможен только после удаления дубля. До прода прогнать на локальной БД на сценарии, где есть и сливаемые данные, и контроли, которые меняться не должны. Необратимая миграция требует бэкапа затронутых таблиц и явного упоминания в запросе «merge and deploy» (`AGENTS.md` §3.2).
...
Модель доступа — `AGENTS.md` §9: только алиас `vf-prod-codex` и wrappers,
read-only по запросу Владельца или как post-deploy acceptance, любая мутация —
отдельное одобрение (§3.3).

Всё остальное — правила доступа, список wrappers, allowlist read-only и
мутирующих команд, порядок добавления разрешений, канонические формы вызова,
обязательный `< /dev/null`, диагностика зависания в песочнице и модель
разрешений `deny > ask > allow` — `docs/maintenance/prod-access.md`.
**Читать до любого обращения к production.**
...
правилами VCS. Он **достижимо зелёный**: сейчас `Found 0 of 2363`, exit 0.
...
(`Found 0 of 2363`, exit 0, перепроверено с `--using-cache=no`). Отдельный
...
- Проверяется машинно: `site/tests/PHPStan/Rules/RepositoryCompanyScopeRule.php`
  ловит публичные методы-запросы Repository (`find*`, `get*`, `count*`, `sum*`,
  `exists*`, `search*`, `fetch*`, `load*`, `has*`, `is*`, `are*`, `max*`, `min*`,
  `paginate*`, `list*`, `all*`, `total*`, `stream*`, `iterate*`) без
  `$companyId`/`$company`.
  `save()` и `remove()` под правило не попадают: сущность уже несёт компанию.
  Правило применяется только к репозиториям, чья **сущность действительно
  принадлежит компании** — у справочников платформы (тарифы Billing, внешние
  категории, боты) ограничивать выборку нечем, и требовать от них `companyId`
  значило бы пометить недостижимое состояние. Принадлежность выводится из
  модели, а не из списка исключений.
- Ограничением считается **только** явный параметр: `string $companyId` либо
  объектный `$company`, и тип обязан быть ненулевым. Компания, спрятанная
  внутри DTO, ограничением не считается — правило не гадает о семантике
- Метод, который обязан ходить по всем компаниям (или ограничивает выборку
  иначе), объявляет это явно: `@companyScopeExempt <причина>` в docblock.
  Причина обязательна и читается в ревью рядом с кодом. Это единственный
  **санкционированный** способ подавить срабатывание — и он оставляет след.
  Ниже перечислено, чего правило не видит вовсе: там подавлять нечего
- На момент внедрения — 51 нарушение в baseline
- **Правило — эвристика, а не доказательство.** Оно не проверяет: использование
  параметра (неиспользуемый `string $companyId` пройдёт), владение глубже одного
  уровня, методы с именем вне списка префиксов, и вызовы унаследованных от
  Doctrine `find()`/`findBy()`/`findOneBy()`/`findAll()`/`count()` в местах
  использования (84 таких вызова в `src` — отдельное call-site правило в
  FOLLOW-UP). Полный список ограничений — в докблоке самого правила.
  Зелёный гейт не означает отсутствия IDOR; ручную проверку он сокращает,
  но не отменяет
...
Критерии и порядок — `AGENTS.md` §5 «Execution hierarchy» и §3.2 «merge +
deploy». Здесь не пересказывать: при расхождении пересказа
