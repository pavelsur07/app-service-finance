...
включена branch protection; обязательны семь проверок: `🔬 Static analysis`,
`🎨 PHP code style`, `🧪 Unit tests`, `🧪 Integration tests` и три
`🧪 Functional tests N/3`. Красная проверка блокирует merge.

Отсюда следует правило, которое легко нарушить молча. **Имя job'а в
`.github/workflows/deploy.yml` — это контракт с защитой ветки.** Переименовал job
или изменил число functional-шардов — обязательная проверка со старым именем
больше не появится никогда, и PR встанет навсегда, потому что удовлетворить её
нечем. Список обязательных проверок обновляется **в том же PR**, что меняет имена:

```bash
gh api repos/<owner>/<repo>/branches/master/protection/required_status_checks \
  -X PATCH -f 'contexts[]=<новое имя>' ...
```

Пропуск job'а по `if:` безопасен: workflow запускается на каждый PR (`paths`-фильтра
на уровне `on:` нет), поэтому check всегда появляется, а пропущенный GitHub
засчитывает как пройденный. Фронтенд-only и docs-only PR из-за этого не блокируются.
