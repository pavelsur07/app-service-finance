...
- явно запрошенной post-merge Git-уборки после проверки точной смерженной
  ветки: локальной, а также remote-ветки, если Владелец назвал её по имени
  (см. «Удаление ветки»);
...
Каждый PR создавать с явной базой `master`; при `gh pr create` — передавать
`--base master`. Затем сверять сохранённое значение: `gh pr view <n> --json
baseRefName`.

База, отличная от `master`, — дефект, а не вариант. PR смержится в feature-ветку,
GitHub честно покажет `MERGED`, но сам по себе такой PR не доставляет в `master`
ничего. Изменение всё ещё может доехать другим путём — ту feature-ветку смержат
позже, cherry-pick, отдельный PR, — но каждый такой путь требует собственного
доказательства, а без него работа теряется молча. Ни список веток, ни состояние
PR не отличают это от
настоящего мержа, поэтому `baseRefName` нужно проверять явно. Чинится командой
`gh pr edit <n> --base master` до мержа, после чего заново проверить полный diff
PR и CI: смена базы меняет содержимое PR.

### Удаление ветки

Удалять ветку только после доказательства, что зафиксированный GitHub результат
мержа находится в текущей истории `master`. Это утверждение об истории, а не о
рабочем дереве: более поздний revert отменит содержимое, оставив merge-коммит
предком.
...
Удаление разрешено, только когда `state` равен `MERGED`, `baseRefName` равен
`master`, `headRefName` — это именно удаляемая ветка, `mergeCommit` является
предком свежеобновлённого `refs/remotes/origin/master`, а `headRefOid` до сих
пор совпадает с вершиной ветки. Проверка вершины — ловушка: в ветке, куда
дописали после мержа, PR остаётся `MERGED`, а новые коммиты до `master` не
доехали. Если head-ветка PR живёт в другом репозитории, в `origin` её нет —
на основании такого PR там не удалять ничего.

Удалять через `git branch`, не через plumbing: `git update-ref -d` снесёт ветку,
занятую worktree, и оставит в этом worktree `HEAD`, указывающий на удалённую
ссылку. Предпочитать `-d`, но не считать доказательством ни его успех, ни его
отказ: он проверяет вершину относительно настроенного upstream, а при его
отсутствии — относительно `HEAD`, и про `master` не говорит ничего ни в одну
сторону. Если upstream указывает на собственную `origin/<branch>`, `-d` спокойно
удалит ветку, которая никуда не доехала; если upstream нет, а `HEAD` на `master`,
он отвергнет squash-смерженную ветку с доставленным содержимым — её исходные
коммиты не стали предками squash-коммита. Доказательство — факты выше, а `-d`
только инструмент. Когда все факты сошлись, а `-d` всё равно отказывает,
правильный инструмент — `git branch -D`: та же защита worktree сохраняется.
Запрещено именно тянуться к `-D` без этих фактов.

Удаление remote-ветки никогда не входит в автономную уборку: нужна явная
инструкция Владельца с именем этой ветки. Непосредственно перед удалением
перечитать живую вершину — первое поле
`git ls-remote --heads origin refs/heads/<branch>`, а не потенциально устаревший
`origin/<branch>` — и потребовать, чтобы она по-прежнему равнялась `headRefOid`.
Любое расхождение отменяет удаление.

Проверка и удаление — два шага, поэтому в принципе push, попавший между ними,
потеряется. Принято сознательно: в этом репозитории пушит один человек, и он же
даёт команду на удаление, так что второму писателю взяться неоткуда. Не
скриптовать и не удалять пакетно — именно запуск без присмотра или по списку
превратил бы это окно в настоящее.
