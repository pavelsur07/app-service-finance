...
- явно запрошенной post-merge Git-уборки после проверки точной смерженной
  ветки — только локальной; remote-ветку агент не удаляет, а сообщает Владельцу
  (см. «Удаление ветки»);
...
Remote-ветки агент не удаляет вообще — это делает Владелец. Проверка живой
вершины и удаление — разные шаги, а обычный `git push --delete` сносит ветку по
имени независимо от того, что успело прийти между ними, поэтому рецепт не может
гарантировать обещанного. Сообщить, какие remote-ветки подходят под удаление, и
оставить само удаление Владельцу. Вернуться к вопросу только после того, как в
этом репозитории будет реально проверено атомарное условное удаление,
отказывающее при изменившейся вершине.
