...
2. **Повторное использование прежде нового кода.** Перед написанием функции,
   helper'а, DTO или сервиса — поискать существующее решение в кодовой базе
   (Grep по модулю и соседним, `PATTERNS.md`, Facade'и). Уже есть — расширить
   или вызвать, а не написать дубликат в паре файлов рядом. Не создавать
   абстракции без запроса: интерфейс с одной реализацией, фабрика одного
   продукта, конфиг неизменяемого значения — не нужны. Выигрывает кратчайший
   рабочий дифф в правильном месте.
3. Модуль вывести из соседнего кода; спросить только при материальной
   неоднозначности.
4. Новый Facade, метод, Enum или Entity, однозначно следующий из ТЗ, — создать
   в scope и обновить `ARCHITECTURE.md` в том же Stage.
5. Нужен паттерн — соответствующий раздел `PATTERNS.md`.
