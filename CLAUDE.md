...
**Красный baseline — не регресс, но и не индульгенция.** Репозиторная проверка может оказаться красной до задачи. В этом случае проверять точечно только изменённые файлы, сравнивать с их состоянием до правки и записывать в отчёт факт красного baseline с цифрой. Не выдавать чужие нарушения за свои и не прятать за ними свои. Проверено 2026-08-30: `make site-cs-check` и `make site-cs-strict-types` сейчас **зелёные** (`Found 0 of 2346`, exit 0; cs-check перепроверен с `--using-cache=no` — тёплый кэш даёт ложную зелень). Статический анализ — `make site-stan` (PHPStan, level 8). Он достижимо зелёный: унаследованные ошибки зафиксированы в `site/phpstan-baseline.neon` (3525 после подключения расширений Symfony/Doctrine/PHPUnit/webmozart), анализируется 2344 из 2346 файлов `src`+`tests`; два тестовых файла временно исключены — см. комментарий в `site/phpstan.dist.neon`. Расширения читают контейнер из `var/cache/test`, поэтому `make site-stan` сначала прогревает его сам. Baseline только сокращать — перегенерация «начисто» (`make site-stan-baseline`) стирает ratchet и делает регресс неотличимым от долга. Таргета `make stan` без префикса в проекте нет.
...
правилами VCS. Он **достижимо зелёный**: сейчас `Found 0 of 2346`, exit 0.
...
(`Found 0 of 2346`, exit 0, перепроверено с `--using-cache=no`). Отдельный
